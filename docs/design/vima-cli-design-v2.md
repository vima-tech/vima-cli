...
3. **批次调度（核心循环）**
   ...
   - **批次检查点**：本批全部通过后执行 git commit（格式：vima: batch <N> completed），形成批粒度回滚点。
     用户输入 /go 即构成对本次运行全部检查点提交的明确授权，不逐批征询；提交被环境
     规则/权限拒绝时跳过并在批次报告注明「未形成回滚点」，不中断调度（A17）
   - **会话预算**：单次 /go 最多推进 8 个任务（按任务计数，批次数不设上限——
...
| 策略 | 做法 |
|------|------|
| **单次 /go 任务预算** | 单次会话最多推进 8 个任务（按任务计数，批次数不设上限——串行批下按批计数会在 3 个任务后过早截断，A17），达标后主动落盘并提示："本会话已完成 X 个任务，请再次输入 /go 继续" |
