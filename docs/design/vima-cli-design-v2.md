...
>
> 当前修订：**v2.0.8**（标题中的 v2.0 指主版本线，逐次修订见 §21 版本历史与文末页脚）。
...
|------|---------|------|---------|---------|
| `admin`（优先） | `spec.md`（九章：概述/数据模型/页面/接口/规则/权限/技术栈/关键决策记录/本期不做，第八章为 A4 增补、第九章为 A13 增补） | ✅ `docs/contracts/*.md` | 前端 + 后端两种 | v2.0 全量落地 |
...
└── planning/                  # 规范体系资产
    ...
    ├── spec.admin.md          # spec.md 骨架模板（九章结构，含各章必填要素标注）
...
- **E. 业务规则**：校验规则、状态流转、计算规则、约束条件——逐条结构化写入 `vima:rules` 数据块（A13：id/type/entity/desc + 可选 apis），散文仅作人读说明，机器真源是数据块
- **F. 权限设计**：角色清单、**每个角色的菜单权限清单**、操作权限、数据权限；无任何角色覆盖的菜单必须显式标记
- **G. 技术约束**：前后端技术栈、脚手架命令、UI 框架信息
- **H. 本期不做**（A13）：本期明确不实现的事，写入第九章 `vima:non-goals`（NG-xx + desc）；确实没有也必须显式写 `non-goals: []`——范围的本质是边界不是清单
...
| 文档 | 位置 | 作用 |
|------|------|------|
| `spec.md` | `docs/spec.md` | 系统概述、技术栈、数据模型、页面清单（页面级粒度）、业务规则（A13 结构化 `vima:rules`）、权限矩阵、业务流程、关键决策记录（A4）、本期不做（A13 `vima:non-goals`） |
...
2. **任务分析（CLI 确定性，不劳 Agent 拓扑计算）**
   ...
   - 计划内容：批次 0 为所有 layer=shared 的任务（串行）；批次 1..N 为按 dependsOn
     拓扑分批的 layer=business 任务（批内并行，单批 ≤ maxParallel，默认 8 且可配，A18）；
     批次末为 layer=pipeline 的顺序流水线。每批带 level 字段——同 layer 同 level 的
     批次之间无依赖，主 Agent 可流水线化派发（上批 Verifier 与下批 Builder 同轮，A18）
...
3. **批次调度（核心循环）**
   ...
   - **批次检查点（A18 授权口径，取代 A17）**：只在 `/go --commit` 时执行
     git commit（格式：vima: batch <N> completed），形成批粒度回滚点；该 flag 即构成
     对本次运行全部检查点提交的明确授权，不逐批征询。**不带 --commit 时完全不碰 git**
     （与用户环境级提交禁令解耦）；带 flag 而提交仍被拒时跳过并注明，不中断调度（A17 保留）
   - **会话预算**：单次 /go 最多推进 24 个任务（按任务计数，批次数不设上限——
     shared/pipeline 串行批每批仅 1 任务，按批计数会在 3 个任务后过早截断，A17；
     阈值 8→24 见 A18，前提是子代理回传摘要 ≤15 行、明细落盘），达标后落盘并提示用户
     **先 /clear 再 /go** 续跑（同会话重输 /go 不重置上下文，预算形同虚设，A18）
   - **合法停点白名单（A17）**：批次之间不得停轮等待用户；唯一允许的停点 =
     预算耗尽 / 全部任务终态 / 确定性前置失败或闸门阻断需用户处置（lifecycle 缺失、
     plan 依赖环、三道闸门、failed 任务裁定）/ 用户主动中断
   - 若还有未完成批次且未被阻断 → 派发下一批
...
apis: ['GET /api/device/list']             # 本任务负责的契约接口子集（A18，可选；缺省=契约全集）
...
| `apis` | A18 可选：本任务负责的契约接口子集（`METHOD /path` 数组），缺省 = 负责该契约全部接口。契约超 10 个接口时按子域拆任务并各自声明（V-TASK-11 规模上限 / V-TASK-12 ⊆契约·不重叠·并集齐全）；`vima context` 据此切片契约 |
...
4. **批内并行度上限**：单批默认不超过 8 个 Builder（A18 由 5 提高并做成 `vima plan --max-parallel` 可配项——原值 5 与新值 8 同为取值判断，故给出配置入口），超出时拆成多个子批次派发；同层子批之间无依赖，允许流水线化重叠（A18）。
...
| 策略 | 做法 |
|------|------|
| **单次 /go 任务预算** | 单次会话最多推进 24 个任务（按任务计数，批次数不设上限，A17；阈值 8→24 见 A18——前提是回传摘要 ≤15 行使每任务上下文成本有界），达标后主动落盘并提示："本会话已完成 X 个任务，请先 /clear 再输入 /go 继续"（A18：同会话重输 /go 不重置上下文，预算不起保护作用） |
...
**骨架先行**：`vima init` 安装的 spec 骨架（admin 为九章结构）在 PLANNING 开始时复制为 `docs/spec.md`，每章预置标题、必填要素标注与填写提示。Agent 的工作从"创作一份文档"降级为"逐章填空"，漏章在结构上即刻可见；每章写完即落盘并更新 checkpoint，天然支持跨会话断点续写，长文档截断风险降为单章级别。

**机械校验清单（validate.checklist.md，模板资产）**：以 admin 模板为例，至少覆盖：

- spec.md：九章齐全；数据模型每个实体含字段表；每个枚举/字典列出全部取值；接口清单与契约一一对应；**每个页面含页面级粒度四要素**（布局拆分/组件清单/交互设计/接口映射，YAML 数据块可解析）；**布局使用枚举区块词表，跳转目标与弹窗 ID 存在且唯一**；角色章节含菜单权限清单且无权限盲区；**第五章业务规则逐条结构化入 `vima:rules`**（type/entity 合法、apis 落在契约上）；**第九章 `vima:non-goals` 显式声明**（空清单亦须写出）
...
| **ID 交叉引用** | 角色/菜单/页面/接口/流程/规则/不做项均带稳定 ID（ROLE-xx/MENU-xx/PAGE-xx/FLOW-xx/RULE-xx/NG-xx，后两者 A13），视图间悬停预览与跳转 | 
...
| 大型（30+ 任务） | 70+ | 建议拆多个 /go 会话推进，配合任务预算（A17）自然达成 |

- 总成本约为单 Agent 顺序开发的 1.5-2 倍（Verifier 与重试是额外开销），换来的是批内并行压缩总耗时与独立校验带来的质量保障；
- 任务预算设计不增加总成本，只是把同样的工作量切分到多个会话执行；
...
确定性生成批次计划（见 7.5 步骤 2，主 Agent 不自行拓扑计算）：
  ...
  - 输出 .vima/reports/batch-plan.json：批次划分、批内任务清单、每批 level（A18 流水线化判据）、并行度上限（单批 ≤ maxParallel，默认 8，`--max-parallel <1..10>` 可配，A18）
  - 只读操作，不修改任何状态
...
用户评审的机械确认（6.4 步骤 13，第三道闸门的落痕动作）：
  - 前置：vima validate 通过且审计视图/原型已渲染且与当前 spec 无漂移
    （A12 新鲜度机检，复用渲染器 --check 的逐字节比对语义），否则中止
...
用户问「已有项目能否通过 `vima update` 升到最新功能而不影响原有程序」。核实后确认主体已实现
（update 受管清单里代码文件命中数为 0；改过 210 个代码文件的项目跑 update 后代码树指纹一致），
补齐三处缺口：

- **manifest v1→v2 端册迁移**：兑现契约 §6.4 早已写下却从未实现的宣称。迁移带**保护面不降级**
  的兜底——后端共享层按模板声明渲染并逐个校验在位，缺一个就放弃迁移保住 v1 字面量兜底
- **doctor 第 ⑫ 项产物形态体检**：A4 决策表 / A13 两个数据块 / A2 任务 page 四条判据，
  级别与 validate 对齐；告诉老项目「缺的是哪个增补项引入的、补在哪一章」，
  而不是等 `/go` 闸门才撞上 error
- **骨架基线 + `update --scaffold-diff`**：create 记录 `files.scaffold` 落盘哈希，
  只读三方比较报出「可安全更新 / 需人工」；**只报告不写盘**——骨架即业务代码，
  自动改它与「不影响原有程序」直接冲突（故不提供 `--scaffold-apply`）

**（二）批次调度效率（A18）**

sustain-v3 实测评估（`docs/design/batching-efficiency-assessment.md`）触发 A17 自留的
重开条件。实测证伪了两个直觉归因：构建不是瓶颈（前端 build:check 2.82s、后端
mvn compile 1.34s），工作量也不是（总生成量 120 分钟是硬成本）。真因是**不均衡造成的
空转**——子代理内部严格串行，批次时长取批内最大值，而单任务最大 4527 行 = 同批最小
任务的 7 倍，实测三个业务批的并行槽空转率 52–54%。

- **任务规模上限 + 负责接口集**：frontmatter 新增可选 `apis`（§9.2），V-TASK-11（>10 接口
  warn）/ V-TASK-12（⊆契约·不重叠·并集齐全 error）；`vima context` 按 `apis` 切片契约。
  拆分能力与其完整性机检同批落地——拆分本身会引入「漏实现/重复实现」这一新失败模式
- **前端任务默认只依赖 shared-base**：契约先行的必然推论（前端验收清单不含任何后端
  运行时依赖）；旧默认把 50 个前端任务锁到 19 个后端任务之后，凭空多出一半批次
- **同层子批流水线化**：batch-plan 每批带 `level`（§19.9），同 layer 同 level 的批次之间
  无依赖，允许「上批 Verifier + 下批 Builder」同轮派发，2N 轮压到 N+1 轮
- **并行度可配**：`vima plan --max-parallel <1..10>`，默认 5 → 8（新增错误码 PLAN_PARALLEL）
- **预算 8 → 24**：前提是角色模板规定回传摘要 ≤15 行、明细落 `.vima/reports/`；
  续跑提示改为「先 /clear 再 /go」——同会话重输 /go 不重置上下文，预算原本形同虚设
- **检查点提交改 `/go --commit` 显式授权**（取代 A17「/go 即授权」）：不带 flag 完全不碰
  git。实测 sustain-v3 至今 0 提交，A17 口径被用户全局提交禁令压制而始终不生效
- **Stop hook 续跑器**：`.vima/go-state.json`（契约 §6.12）+ `hooks/go-continue.mjs`。
  **推翻 A17「不用 Stop hook」**——A17 的否决理由是 hook 无法区分合法停点与非法停顿，
  让主 Agent 把停因写成机读文件后该前提不再成立；任何异常一律放行，退化为原行为
...
依「快速理解业务系统的核心要素 / 产品经理关注点」专题讨论结论，经用户裁定立项。
两条判断：角色/场景/流程覆盖行为面，缺状态面与约束面；产品经理关注点的问题层
不在脚手架问题域（终点清单全落解法层是刻意且正确的）。由此只取两处能落成
确定性机检的缺口：

- **业务规则结构化（`vima:rules`，第五章）**：业务规则曾是 spec 里唯一没有结构化块的
  必填内容——机器读不到，于是进不了 validate、进不了审计视图，**也进不了
  `vima context`**（Builder 施工时根本看不见业务规则）。字段口径经用户裁定为
  `entity` 必填（语义归属 + 引用机检）+ `apis` 可选（消费端过滤，前后端任务共同的
  join key；省略 = 全局规则）。新增 V-SPEC-09/10，RULE-xx 并入 V-SPEC-05 ID 唯一性
- **本期不做（`vima:non-goals`，新增第九章）**：spec 八章全是「要做什么」，边界只靠
  「没写就不做」的隐性约定，而生成式模型最擅长补全没写的东西。新增 V-SPEC-11 强制
  **显式声明**——空清单也须写 `non-goals: []`，「声明为空」与「没声明」必须可区分。
  落点经用户裁定为独立第九章（与第八章决策记录分量对等），spec 八章→九章
- **消费端一次接齐**（CLAUDE.md 新增硬约束「立项即做透」）：validate 机检 /
  `vima context` 新增「业务规则切片」与「本期不做」两节（切片按 apis 交集 + 全局规则
  确定性过滤）/ 审计视图第⑤视图与红线区（审核指引四步→五步）/ Verifier 逐条核对
  RULE-xx 并对越界记 `NG-xx 越界` fail（复用 §6.9 points，不改报告 schema；
  越界不适用 waived——边界真源是 spec 而非对话）
- **刻意不做**（防后人当遗漏补上）：不引入实体状态机块（admin 域无第二实例，YAGNI；
  `type: transition` 已能表达单条迁移约束）；不引入问题层要素（成功度量/价值主张
  落不成机检规则，进 spec 只会是摆设）
- 顺带订正：§13.1 两处遗留的「spec 七章」表述（A4 改八章时未回写）随本次统一为九章；
  契约 §7 `vima:flow` 的「第七章」标注订正为「第五章业务流程小节」

### v2.0.7 (2026-08-13) 原型先行节拍（增补项 A12 落地）

依 mattpocock-skills-assessment.md §6「原型先行」时序分析，经用户裁定采纳：

- **逐模块「草→渲→看→定」节拍**：planning-guide §5 里程碑 2 改造——模块页面块草入
  spec 即渲染原型，请用户对图反应、修正回 YAML、重渲染；页面对齐完成判据从
  「谈到页面级粒度」升级为「用户在原型上看过并确认」；§7 终点评审降级为最后一次
  全量过目。原型从终点验收物变为过程催化物，A2 单一真源不被触碰（改的永远是 YAML）
- **approve 前置 2 新鲜度机检**：从「产物存在」升级为「产物与当前 spec 渲染结果
  逐字节一致」——render-review/-prototype 导出 `checkReviewFresh` /
  `checkPrototypeFresh`（契约 §11，与 --check 共用 util/fs `driftOf` 比对，
  漂移判定单一真源），漂移 exit 4 并指名重渲命令；顺带修掉「渲染后改 spec
  不重渲也能 approve」的潜伏洞。approve 单测改为真实渲染 + 漂移 e2e

### v2.0.6 (2026-08-13) mattpocock/skills 对标采纳（增补项 A9–A11 落地）

依 docs/design/mattpocock-skills-assessment.md（35 个 skill 逐篇研读）采纳清单的
定向落地——三项均为文字资产级吸收（不新增命令/文件/报告格式，判据全部进 d2
防漂移断言），编排层整体进不吸收清单（vima 已用机械形式承担同等职能）：

- **A9 提问三规则**（吸收自 grilling）：planning-guide §5 新增 frontier 提问纪律
  （先查后问 / 一轮问全＋每问必附推荐答案 / 前置未定不问），vima-planner 纪律镜像；
  推荐答案经用户确认后才升级为「明确确认」信息源，与信息源分级协同
- **A10 同构断言禁令**（吸收自 tdd 反模式）：coding-standards 后端节新增
  〔L5·verifier〕条——单测期望值必须来自独立事实源（契约示例/spec 业务规则/已知值），
  同构断言视同无测试；`_template-be` 步骤 5 同步措辞，防 A7 做实的 `mvn test`
  信号被同构断言无声退化回恒绿假门
- **A11 红绿修复纪律**（吸收自 diagnosing-bugs Phase 1 硬门）：CLAUDE.project.md
  工作协议新增维护期修 bug 协议——先固化一条能跑红的命令（A7 信号源：失败测试 /
  build:check / runtime-errors.jsonl 复现），修复判定 = 同一命令转绿
- 契约 §12 增补对应条目；本文页脚与「当前修订」自 v2.0.4 修正为 v2.0.6
  （v2.0.5 落地时未同步，属如实表述修正）
...
- 编排会话管理：单次 /go 任务预算 + 续跑切短主会话，解决编排者自身上下文增长
...
- 人类审计视图（13.2，参考 PACT）：双轨产物 + 页面级粒度四要素强制 + vima render-review 确定性渲染单文件 HTML（角色权限/菜单功能点/业务流程串联/页面 UI 详情/业务规则五视图 + 本期不做红线区）
