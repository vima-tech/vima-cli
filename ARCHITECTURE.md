...
设计依据：需求基线 R1–R11 与约束 C1–C4。**正本在 `docs/requirements-baseline.md`**，
本节是它的索引。**每处实现必须能反查到某条需求，反查不到的不写。**
...
**「判据」列不是装饰。** 基线原文的立身之本就是那句「每条附判据——怎么算满足，
而不只是想要什么」。这张表曾经只有「要点」没有「判据」，而**丢掉判据的那几条，
对应实现也确实没人做**（R5 的并发可观测、R7 的骨架不豁免自己、R11 的拿错端机检，
三条当时都是零实现）。判据不在册，就没有人会发现它没做。
`z.seams` 有一条机检盯着这张表：**每条 R#/C# 的判据列必须非空**。

编号有空档是**故意的**：R4 / R8 / R10 已被 R2 吸收，编号不回收，
否则旧讨论里的引用会指向另一条需求。

落地状态三档：**✅ 判据已兑现** · **⚠️ 判据成立但欠着**（不是「不做」）· **📝 判据已改判**（附理由）。
完整的裁定、边界、实证与欠账明细在正本里；两处冲突**以正本为准**。

| 编号 | 需求 | 判据（怎么算满足） | 落地 |
|---|---|---|---|
| **R1** | 从零散物料产出能跑的系统 | **冷读通过**——新起一个 agent 只给规格，看它能否开工、有无追问。冷读**作度量不作闸门** | ✅出处进 claim / ⚠️冷读度量无采集口 |
| **R2** | 观测平台（**最高优先**） | 五类问题**在同一个界面上**都能回答，且数据同源可对账。吸收原 R4/R8/R10 | ✅ |
| **R3** | 改需求，影响面**算出来** | 改完的标志是**失效清单清空**，不是人觉得改完了 | ✅ |
| **R5** | 并行加速 | 实际并发数**可观测**，且与计划并发数**可对账** | 📝后半句已改判（v4 无「计划并发」对象）/ ✅前半句已做 |
| **R6** | 定位根因 | 任意症状能回溯到**第一条证据不足或与上游矛盾**的地方 | ✅ |
| **R7** | 企业风格化四层 | 四层各有**封闭**词表或阶梯 · **骨架自身也满足，不得豁免自己** | ✅四层封闭 / ⚠️骨架豁免了自己、Claude Design 两档未接线 |
| **R9** | 存量项目接管 | 陌生库接管后能产出完整命题图，且反推的命题**来源可标最高档、验证强度不得自动继承** | ✅（双轴正交即此条） |
| **R11** | 可复用业务块库 | 加块**不需要改内核** · 块不含 L4 也能被完整消费 · **拿错端的块是机检项** | ✅三条判据齐 / ⚠️内容侧只有 1 个块 |

| 编号 | 约束 | 判据 / 含义 | 落地 |
|---|---|---|---|
| **C1** | 执行者会偷懒、会自称完成 | 一切「做完了」都要有系统生成的证据；自称永远是最弱档 | ✅（边界见纪律 0） |
| **C2** | 防过度设计 | 约束别扩面 · 抽象别提前 · 字段别镀金。反查不到需求的复杂度 = 未经授权 | ✅ |
| **C3** | 文档可读、可 diff、可 review | 真源是 markdown，不是数据库；渲染确定性，同一输入字节一致 | ✅ |
| **C4** | 设计与开发阶段**不阻塞** | AI 自行定夺、记 ruling、继续；记录须**显式落盘 · 可见 · 可推翻**（触发失效传播） | ✅ |
...
checkAssets(assetsRoot, config) → { theme, blocks, projectKinds }
  // 已登记的皮/块**真的读得出来吗** + **有没有装错端**。判据就是 loadStyle/readBlock 自己，
  // 不另写一份「文件在不在」——那会是第二个真源。
  // 返回纯事实，error/warn 分级由注入方（front/actions.audit）定：
  //   皮不存在 → error（配置指向一个不存在的东西）
  //   块缺层/读不出 → warn（装了但不完整，不挡交付）
  //   blocks[].sideMatch === 'mismatch' → warn（块好好的，但本项目没有端消费得了它）
  // 「块坏了」与「块装错项目了」是两件事，分开报——处置不同（修资产仓 vs 卸块/补端）。

sideMatchOf(meta, projectKinds) → { sides, sideMatch }
  // R11 判据第三条「拿错端的块是机检项」的唯一判据处。
  // **三态不是布尔**：match / mismatch / unchecked——「没查」必须和「查了没问题」分得开。
  // 零假阳性靠第三态：项目还没登记端（新项目正常态）或块没声明 sides，一律不判。
  // 前者刻意**不产生 finding**（每个新项目必亮的告警会训练人忽略整张表），
  // 后者产生一条 closure-unchecked（资产仓可修，且它让本机检对该块整个失效）。
```
...
status({ cwd, env, now }) → { ok, root, tiers, cost, runs, rulings, concurrency, stale, corrupt }
  // 恒不抛、恒 exit 0：它要可视化的正是「会话开在错目录」这种故障。
  // concurrency（R5 判据前半句「实际并发数可观测」）= { active, expired, holders, corrupt, scope }
  //   只能从**租约**算，不能从 run 事件算：事件流回答「发生过什么」，
  //   此刻谁在干活是租约的职责。
  //   `scope` 是那句边界说明的唯一真源，呈现层原样转述——跨 worktree 的租约互不可见，
  //   一个不说明适用条件的并发数会被读成「并行已经做完了」（见「已知未接线」第 4 条）。
  //   判据后半句「与计划并发数可对账」在 v4 无对应物（不是批次调度模型），已改判，
  //   台账在 docs/requirements-baseline.md。
...
1.5. **供料的「料」几乎是空的。** 机制侧已经齐了（注册表 / lock / `block add` /
   `next` 按层下发正文），但资产仓当前只有 **1 个业务块**（`admin/role-management`），
   `blocks/common/` 与 `blocks/wechat/` 是空目录，`lore/` 只有 README。
   R11 点名的起点三类（后台管理七件套 · 微信生态四件 · 通用能力）一件都没齐。
   **这不是机制缺陷，是内容欠账**——但它让「供料」这条支柱目前供不出东西，
   而使命第一句就是「定形 · **供料** · 验形」。写在这里是因为：机制齐了最容易被
   读成「这条做完了」，而实际拿它去起一个项目时，`vima block list` 只有一个选项。
   补料是内容工作，不是改代码；每补一个块要过 `block.json` 的 `sides`/`depends`
   与 L1–L3 三层齐备，`audit` 会盯着。
