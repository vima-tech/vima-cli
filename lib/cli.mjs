...
const PROJECT_SCOPED = new Set([
  ...
  'app', 'mock', 'change', 'certify', 'design', 'go', 'status', 'web',
]);
...
//      A35 的过程曲线当场被噪声淹没。
// web 与 status 同理：常驻只读、声明不落盘，写 journal 会让它变成第 N 个状态源。
const JOURNAL_EXEMPT = new Set(['status', 'web']);
...
const COMMANDS = {
  ...
  web: {
    desc: '本地只读视图：进度 / 追溯图 / 端点反查 / 审核产物（A41）',
    usage: 'vima web [options]',
    intro:
      '起一个只绑 127.0.0.1 的只读 HTTP 服务，把 status 的三档进度、trace 的追溯图'
      + '（按任务 / 按端点 / 按文件三张反查索引）与既有审核产物（审计视图 / 线框原型 / '
      + '过程轨迹 / 设计稿）收进同一个入口。终端答得了「进度是多少」，答不了'
      + '「这个报错的端点是谁实现的、属于哪个任务、验收到哪一步」——那是超链接的活。'
      + '**只读**：不写任何文件、不做鉴权、不监听非回环地址、不上报、不跨项目聚合。',
    options: [
      ['--port <n>', `监听端口（默认 5178，只绑 127.0.0.1）`],
      ['--open', '启动后用系统默认浏览器打开'],
    ],
    examples: ['vima web', 'vima web --port 6000 --open'],
  },
  status: {
...
const HELP_GROUPS = [
  ...
  ['开发期（DEVELOPING）', ['go', 'status', 'web', 'plan', 'sync', 'context', 'trace', 'render-journal']],
