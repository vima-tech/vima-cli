...
const PROJECT_SCOPED = new Set([
  ...
  'app', 'mock', 'change', 'certify', 'design',
]);
// 找不到项目根时**拒绝执行**的子集。两个例外各有既定语义，不得被本项误伤：
// init  —— 在尚不是 vima 项目的目录里首次初始化，本就是它的正常路径；
// doctor —— 「非 vima 项目只跑环境与宪法两项并注明」是它声明过的降级能力（§19.4）。
const PROJECT_REQUIRED = new Set([...PROJECT_SCOPED].filter((c) => c !== 'init' && c !== 'doctor'));
...
// `--json` 不在此列：绝大多数命令的 `--json` 是「**同时**输出到 stdout」，仍会落盘；
// 唯一例外 `plan --json` 无零写盘契约，按 A35 边界说明作为可接受噪声保留。
...
function journalCommand(root, cmd, argv, exitCode) {
  if (!root || !journalEnabled() || !existsSync(path.join(root, '.vima'))) return;
  const dryish = argv.some((a) => JOURNAL_READONLY_FLAGS.has(a));
...
const COMMANDS = {
  create: {
    desc: '创建项目骨架（--template admin|cli|script|lib|h5，admin 之外均为 preview；h5 已收编为 admin 的 h5-mobile kind）',
  ...
  'render-journal': {
    desc: '过程轨迹视图：把已发生的过程渲染成人类审核窗口（A36，只读不批）',
  ...
  app: {
    desc: '端册生命周期：查看端册 / 后补一个前端端（A16 一后端 × 多前端）',
    ...
    examples: [
      'vima app list',
      'vima app add patient --kind mp-native',
      'vima app add ph5 --kind h5-mobile --name 患者端H5',
    ],
  },
  mock: {
    desc: '契约驱动的确定性 mock 生成：docs/contracts → .vima/mock/contract-mock.json（A27）',
    usage: 'vima mock',
    intro:
  ...
  converge: {
    desc: '跨任务集成对账：开发完成后的漏实现/重复实现/越界实现收口（A20）',
  ...
  retro: {
    desc: '项目复盘采集：把跑完的项目经验整理成可反哺 vima-cli 的 issue 正文（A21）',
  ...
  change: {
    desc: '维护期变更事务：基线快照 → 确定性影响面 → done 任务重开 → 传播闸门（A31）',
  ...
  design: {
    desc: '视觉真源的确定性侧：设计产物存在性 / 批准摘要 / 验收报告汇总（A34；MCP 与浏览器不在内核）',
  ...
  certify: {
    desc: '交付等级认证：四级证据阶梯，显式不宣称 deployable/stable（A32）',
