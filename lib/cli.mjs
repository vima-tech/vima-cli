...
const COMMANDS = {
  create: {
    ...
    examples: ['vima create my-project', 'vima create my-project --template admin', 'vima create my-project --no-git'],
  },
  init: {
  ...
  upgrade: {
    desc: '升级 vima 生成物到当前 CLI 版本（manifest 三方比较，用户文件永不覆盖）',
    usage: 'vima upgrade [options]',
    intro:
      '三方比较（记录校验和 / 磁盘现状 / 新模板源）：用户改过的 managed 文件在旁路写 <path>.vima-new 供自行合并，userOwned 永不触碰。',
    options: [
      ['--dry-run', '只输出动作预览表，不写盘'],
      ['--yes', '兼容保留（实现恒非交互，无额外行为）'],
    ],
    examples: ['vima upgrade --dry-run', 'vima upgrade'],
  },
  doctor: {
    desc: '体检：环境依赖 / 宪法行数 / 状态一致性 / 对齐产物漂移',
    usage: 'vima doctor [options]',
    intro: '九项体检（非 vima 项目仅执行环境与宪法两项）；存在 ❌ 项 → exit 2；CLAUDE.md 超 50 行为告警级。',
    options: [
  ...
  'render-prototype': {
    desc: '渲染无样式线框原型 + prototype.manifest.json（--check 验漂移）',
    usage: 'vima render-prototype [options]',
    intro: '语义占位线框 + 管理后台外壳，同时产出 manifest 供 Verifier 对账；模板声明 prototype:false 时跳过。',
    options: [
      ['--check', '不写盘，与磁盘产物字节级比对（不一致 → exit 2）'],
      ['--output <path>', '输出路径（默认 docs/review/prototype.html）'],
    ],
    examples: ['vima render-prototype', 'vima render-prototype --check'],
  },
  sync: {
  ...
  plan: {
    ...
    intro:
      '共享层串行 → 业务层按依赖分层批内并行（≤5）→ 流水线收尾；成环/缺依赖 → exit 2；默认写 .vima/reports/batch-plan.json。',
    options: [['--json', '批次计划输出到 stdout（不落盘）']],
    examples: ['vima plan', 'vima plan --json'],
  },
  approve: {
    desc: '任务评审机械确认：置 tasksApproved（/go 前置闸门）',
    usage: 'vima approve',
    intro: '前置：validate 通过 + 审计视图/原型已渲染 + pendingConfirm 清零；通过后输出任务汇总表并写 lifecycle 留痕。',
    options: [],
