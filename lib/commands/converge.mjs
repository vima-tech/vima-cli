 ...
 */
async function collectVerifierOutcomes(root) {
  const dir = path.join(root, REPORTS_DIR);
  let names;
  try {
    names = await readdir(dir);
  } catch (err) {
    if (err.code === 'ENOENT') return { openPoints: [], contractGaps: [] };
    throw err;
  }
  const openPoints = [];
  const contractGaps = [];
  ...
  return { openPoints, contractGaps };
}
...
export async function evaluateConvergence(root, { cliRoot }) {
  ...
  // ── 既有红信号收口 ──
  const { openPoints, contractGaps } = await collectVerifierOutcomes(root);
...
export async function run(argv, ctx) {
  ...
  const { openPoints, contractGaps, unmarkedDone, findings } = report;
  ...
  if (opts.json) {
    ...
    const lines = [
      ...
      `   error ${errors.length} │ warn ${warnings.length} │ 未过点位 ${openPoints.length} │ 契约缺口 ${contractGaps.length} │ 运行时错误 ${runtimeErrors} │ done 无标注 ${unmarkedDone.length}`,
      `   报告：${REPORT_REL}`,
    ];
