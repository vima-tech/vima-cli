...
import { VimaError, EXIT, usageError } from '../util/errors.mjs';
...
/** --artifact 路径 → 规则组（spec/contracts/tasks/coverage）。 */
function resolveGroups(artifact) {
  const all = { spec: true, contracts: true, tasks: true, coverage: true };
  if (!artifact) return all;
  const norm = artifact.split(path.sep).join('/').replace(/^\.\//, '');
  if (norm.endsWith('spec.md')) return { spec: true, contracts: false, tasks: false, coverage: false };
  if (norm.includes('docs/contracts')) return { spec: false, contracts: true, tasks: false, coverage: false };
  if (norm.includes('docs/tasks')) return { spec: false, contracts: false, tasks: true, coverage: false };
  if (norm.endsWith('coverage-matrix.md')) return { spec: false, contracts: false, tasks: false, coverage: true };
...
export async function run(argv, ctx) {
  ...
  // stdout：分组打印 ❌ error / ⚠️ warn / 待确认，末行统计
  const out = [];
  if (result.errors.length > 0) {
    out.push('❌ 错误：');
    for (const e of result.errors) out.push(`  ${formatLine(e)}`);
  }
  if (result.warnings.length > 0) {
    out.push('⚠️ 警告：');
    for (const w of result.warnings) out.push(`  ${formatLine(w)}`);
  }
