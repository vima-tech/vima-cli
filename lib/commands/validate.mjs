...
import { extractBlocks, hasCheckbox, splitMarkdownTables } from '../util/md.mjs';
...
/**
 * V-TASK-07（warn，B3 覆盖度；A42 D-A42-01 改为按页聚合）：
 * 同一 `page` 的全部任务复选框数之和 < 该页任务点数 → 每页一条。
 *
 * 逐任务判定对「一页一任务」成立，对**合并页/切片任务**不成立——而合并页是 vima 自己
 * 支持并鼓励的形态（A16 端册 + A22 字段级机检）。实测（sustain-v4）：PAGE-03 被 5 个任务
 * 分片，`24+22+28+32+27=133` 与该页任务点数完全一致、双向差集为空，却报了 4 条 warn。
 * 判据来源与 V-TASK-11 的 done 豁免同源（见上方注释）：**产生永远无法清除的告警，
 * 比不告警更有害**——它训练人忽略整张告警表。
 *
 * 只聚合本条：V-TASK-06（page 引用存在性）等其余规则仍逐任务判定，一个任务写错一处即报。
 */
function checkPageCoverage(pageTasks, spec, warnings) {
  for (const pageId of [...pageTasks.keys()].sort()) {
    const group = pageTasks.get(pageId).slice().sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
    const points = countPagePoints(spec.pages.get(pageId));
    const total = group.reduce((n, g) => n + g.boxes, 0);
    if (total >= points) continue;
    const roster = group.map((g) => `${g.id}（${g.boxes} 项）`).join('、');
    warnings.push(entry(
      'V-TASK-07',
      `本页任务验收清单合计仅 ${total} 项，少于页面 ${pageId} 的任务点数 ${points}`
        + `（交互 + 弹窗字段）——参与本页的任务：${roster}；请逐点核对是否漏写`
        + '（同一页被多个任务分片时按页聚合，A42 D-A42-01）',
      group[0].file,
    ));
  }
}
...
async function checkTasks(root, tasks, spec, contracts, errors, warnings, roster = null) {
  ...
  const pageTasks = new Map(); // 页面 id → [{id, file, boxes}]（V-TASK-07 按页聚合，A42 D-A42-01）
  const rosterIds = roster ? new Set(roster.apps.map((a) => a.id)) : null;
  for (const t of tasks) {
    ...
    if (t.fm.page !== undefined) {
      ...
      if (!spec) {
        ...
        // V-TASK-07：本任务的复选框数并入所属页面的聚合桶（判定见 checkPageCoverage）
        const boxes = (t.body.match(/- \[[ xX]\]/g) ?? []).length;
        if (!pageTasks.has(t.fm.page)) pageTasks.set(t.fm.page, []);
        pageTasks.get(t.fm.page).push({ id: t.id, file: t.file, boxes });
      }
    }
  }
  checkPageCoverage(pageTasks, spec, warnings); // V-TASK-07（A42 D-A42-01，按页聚合）
}
...
/**
 * 前端请求门面调用的**唯一真源正则**（A41 D-A41-02）。
 *
 * 泛型段排除括号而非 `>`：本项目主流写法是 `request.delete<any, ApiResponse<{...}>>(...)`，
 * **嵌套泛型里有内层 `>`**，旧式 `[^>(]*` 在第一处 `>` 断开、整条调用被静默跳过。
 * 实证（sustain-v4，2026-08-16）：admin 端 281 个调用点里 216 个（77%）带嵌套泛型，
 * V-CODE-01 与 converge 的 V-INT-04 因此长期只看得见不到 1/4 的调用面，而**都显示通过**。
 *
 * 导出为字符串而不是 RegExp 实例：带 `g` 的正则有 `lastIndex` 状态，跨模块共享同一个实例
 * 会让两处扫描互相污染进度。
 * **此前 validate / converge / traceability 各存一份拷贝，本项收成一处**——
 * 三份拷贝正是「修了一处、另两处继续沉默」的成因。
 *
 * **调用方不要直接用这个常量**，用下面的 `scanFeCalls(text)`：正则只是判据的一半，
 * 另一半是「怎么扫」，两者必须一起收口（理由见 scanFeCalls 的注释）。
 * 本常量保持导出仅供单测直接断言正则形状。
 */
export const FE_CALL_SOURCE = String.raw`\brequest\s*\.\s*(get|post|put|delete|patch)\s*(?:<[^()]*?>)?\s*\(\s*(['"\`])([^'"\`\n]*)\2`;

/** 注释行开头：`//`、块注释续行 `*`、jsdoc 起始 `/**`、wxml/vue 模板注释 `<!--`。 */
const COMMENT_LINE_RE = /^\s*(?:\/\/|\/?\*|<!--)/;

/**
 * 扫一段源码里的全部前端请求门面调用——**正则与扫描纪律的唯一真源**（A41 D-A41-02）。
 *
 * 只导出正则是不够的：真源统一后，validate / converge / traceability 仍各自决定
 * 「怎么喂它」，于是又漂出两处不一致（实证，sustain-v4 2026-08-16）——
 *
 * ① **必须按全文扫，不能逐行扫**。泛型一长就会被格式化器折行：
 * ```ts
 * return request.get<unknown, { code: number; msg: string; data: PatientOverview }>(
 *   `/patients/${id}/overview`
 * )
 * ```
 * 方法名、泛型段、路径字面量分处三行，逐行 `matchAll` 一条都命中不了。
 * traceability 曾逐行扫，报「无人调用 39 个端点」，而同一条正则在此处按全文扫只报 2 个。
 *
 * ② **注释行里的反例不算调用**。共享层 request.ts 的文档注释里写着
 * `❌ request.get(\`/mp/${kind}/list\`)  路径拼变量，机检读不出常量部分`
 * ——那是教「别这么写」的示例，却被 V-CODE-01 当成真实的契约外调用报 error。
 * 判据取「匹配起始行去空白后以注释符开头」：真实调用不会写在注释行上，
 * 跨行调用的匹配又总是起始于 `request.xxx` 那一行，故此判据零假阴性。
 *
 * @returns {Array<{method: string, path: string, index: number, line: number}>}
 */
export function scanFeCalls(text) {
  return scanSource(text, FE_CALL_SOURCE).map(({ m, line }) => ({
    method: m[1], path: m[3], index: m.index, line,
  }));
}

/**
 * 全文正则扫描的**共同底座**（A41 D-A41-02 的扫描纪律，A42 复用）：按全文扫（不逐行，
 * 折行写法才不会漏）、给出 1 起的行号、默认跳过注释行里的反例。
 * V-CODE-01（scanFeCalls）/ V-CODE-03 / V-CODE-04 / V-SRC-02 全部走这一条通道——
 * 每条规则再自建一份「怎么扫」正是本轮 F4/F8 的成因。
 *
 * `skipComments: false` 供 V-SRC-02 使用：编号引用**恰恰要连注释一起查**
 * （`// 参见 PAGE-41` 与 `data-page="PAGE-41"` 同为悬空锚点）。
 *
 * @returns {Array<{m: RegExpMatchArray, line: number}>}
 */
function scanSource(text, source, { skipComments = true } = {}) {
  ...
  for (const m of text.matchAll(re)) {
    ...
    if (skipComments) {
      const lineText = text.slice(starts[li], starts[li + 1] ?? text.length);
      if (COMMENT_LINE_RE.test(lineText)) continue;
    }
    out.push({ m, line: li + 1 });
  }
  return out;
}
...
/**
 * 业务代码作用域的**唯一读取口**：V-CODE-01/02/03/04 与 V-SRC-02 同口径（带 `@vima`
 * 标注的文件），且**只读一遍**——五条规则各自 walk 一次目录是同一份事实读五次。
 *
 * @returns {Promise<{frontend: Array<{app: object, files: Array}>, backend: Array, backendDir: string}>}
 */
async function collectMarkedSources(root, roster) {
  const apps =
  ...
  for (const app of apps) {
    ...
    frontend.push({ app, subdir, files: await scanMarkedFiles(root, subdir, FE_EXTS) });
  }
  const backendDir = roster?.backend?.dir ?? 'backend'; // A16：后端目录读端册（单数不变）
  const backend = await scanMarkedFiles(root, `${backendDir}/src`, BE_EXTS);
  return { frontend, backend, backendDir };
}
 ...
 * 故一条正则通吃全部端（契约 §8）。
 */
function checkFrontendCode(sources, contractKeys, errors, consumersByKey = null) {
  for (const { app, files } of sources.frontend) {
    for (const f of files) {
      for (const call of scanFeCalls(f.text)) {
        const key = feApiKey(call.method, call.path);
...
function checkBackendCode(sources, contractKeys, errors) {
  const reMapping = /@(Get|Post|Put|Delete|Patch)Mapping\s*(?:\(([^)]*)\))?/g;
  for (const f of sources.backend) {
...
// ---------------------------------------------------------------------------
// A42 接缝对账（D-A42-02）：V-CODE-03 错误码 / V-CODE-04 权限点 / V-SRC-02 编号引用
// 「同一事实存在多处时必须有一条规则做对账；**对账只做零假阳性的那条**」。
// 三条规则共用 V-CODE 作用域（带 @vima 标注的业务代码，见 collectMarkedSources）。
// ---------------------------------------------------------------------------

/** 文件头部的归属标注 `// @vima <taskId>`（A1 代码级追溯）——取首个，一文件一归属。 */
const MARKER_ID_RE = /@vima\s+([a-z0-9][a-z0-9-]*)/;

/** ErrorCode 枚举常量：`RESOURCE_NOT_FOUND(40401, "资源不存在"),`（名字 → 数字码）。 */
const ERROR_ENUM_CONST_RE = /\b([A-Z][A-Z0-9_]{2,})\s*\(\s*(\d{3,6})\s*[,)]/g;

/** 抛点：`throw new BusinessException(ErrorCode.X)`（折行写法由全文扫描覆盖）。 */
const BIZ_THROW_SOURCE = String.raw`\bBusinessException\s*\(\s*ErrorCode\s*\.\s*([A-Z][A-Z0-9_]*)`;

/**
 * 项目的 `ErrorCode` 枚举（名字 → 数字码）。**不是 vima 骨架资产**——骨架只有
 * `GlobalExceptionHandler` + `ApiResponse.error(code, msg)`，`ErrorCode`/`BusinessException`
 * 由项目自建（sustain-v4 实证）。故：找不到该文件 ⇒ 整条 V-CODE-03 静默跳过，
 * 不对「用别的机制表达错误码」的项目产生任何误报。
 * 同名映射到不同数字（多份枚举）⇒ 该名字整体丢弃（无法归一的不判）。
 */
async function loadErrorCodeEnum(root, backendDir) {
  const dir = path.join(root, backendDir);
  if (!(await fileExists(dir))) return new Map();
  const map = new Map();
  const conflicting = new Set();
  for (const rel of await walkFiles(dir, { exclude: ['node_modules', 'dist', 'target', '.vima', 'vendor'] })) {
    if (!rel.endsWith('ErrorCode.java')) continue;
    const text = await readFile(path.join(dir, rel), 'utf8');
    for (const m of text.matchAll(ERROR_ENUM_CONST_RE)) {
      const prev = map.get(m[1]);
      if (prev !== undefined && prev !== m[2]) conflicting.add(m[1]);
      else map.set(m[1], m[2]);
    }
  }
  for (const name of conflicting) map.delete(name);
  return map;
}

/**
 * V-CODE-03（error）错误码对账·**实现抛了、契约没声明**（A42 D-A42-02）。
 *
 * 判据：带 `@vima <taskId>` 标注的后端文件里 `BusinessException(ErrorCode.X)`，其数字码
 * 必须 ∈ 该任务 `contract` 指向契约的 `errors[].code` 全集。不在 ⇒ 消费端没法处理该错误。
 *
 * **只做这一个方向**（另一向「契约声明了、实现不抛」已论证做不到零假阳性，理由见
 * 增补册 A42 D-A42-02 的落地说明）。四道零假阳性闸门，任一不满足即整体跳过：
 * ① 找不到 `ErrorCode.java` ⇒ 项目不用这套约定；② 任务集为空 ⇒ 无从归属；
 * ③ 文件的 `@vima` 标注找不到对应任务、或该任务无 `contract` ⇒ 无从归属
 *   （底座/共享任务天然落在此闸门）；④ 枚举里查不到该名字 ⇒ 映射不出数字码。
 *
 * 归属取**契约 module 全集**而不是「该任务的 apis 子集」：Service 常被同模块的兄弟
 * 子任务共用，收窄到子集会把「兄弟任务的端点声明了这个码」误判成缺声明（F11 实证
 * `@vima` 标注在附属文件上系统性错标，模块级归属对这类错标免疫）。
 *
 * **第五道闸门：全项目零声明的码降为 warn**（A42 立项后实跑补上）。
 * 实证（sustain-v4）：`40101 UNAUTHENTICATED` 被抛 2 次而全部 22 份契约零声明——
 * 查项目的 `_error-codes.md` 才知道那是**有意的**：「认证失效由系统底座统一拦截，
 * 不进业务契约的 errors[]」。报 error 是假阳性。
 * 但不能简单地「零声明就跳过」：同一轮里 `40902 DUPLICATE` 同样零声明，
 * 而它是**真缺陷**（device/qc 抛重复码、契约全都漏了声明）——跳过会用 2 个假阳性
 * 换 3 个假阴性。
 *
 * 两者的区别写在项目自己的错误码约定文档里，而**那份文档不是 vima 的资产**
 * （`_error-codes.md` 在 templates/ 与 lib/ 中零引用，纯属项目自创）——
 * vima 无从通用判定，硬做就是发明一个它不拥有的约定（违反「抽象别提前」）。
 * 故按证据强度分级：
 *   - 该码在项目**别处的契约里声明过** ⇒ 项目确实把它当业务码 ⇒ 这里漏声明是缺陷 ⇒ **error**
 *   - 该码**全项目零声明** ⇒ 二义 ⇒ **warn**，消息把两种可能都摆出来让人裁定
 * 这样零假 error，且 40902 那类仍然浮出水面。
 */
function checkErrorCodes(sources, tasks, contracts, errorEnum, errors, warnings) {
  if (errorEnum.size === 0 || tasks.length === 0) return;
  const codesByContract = new Map(); // 契约文件 → 该 module 声明过的错误码集合
  for (const c of contracts) {
    const set = new Set();
    for (const api of c.apis ?? []) {
      for (const e of Array.isArray(api.errors) ? api.errors : []) {
        if (e && typeof e === 'object' && (typeof e.code === 'number' || typeof e.code === 'string')) {
          set.add(String(e.code));
        }
      }
    }
    codesByContract.set(c.file, set);
  }
  // 全项目声明过的错误码全集——用于区分「框架码（处处不声明）」与「漏声明的业务码」
  const declaredAnywhere = new Set();
  for (const set of codesByContract.values()) for (const c of set) declaredAnywhere.add(c);
  const taskById = new Map(tasks.map((t) => [t.id, t]));

  for (const f of sources.backend) {
    const marker = MARKER_ID_RE.exec(f.text);
    const t = marker ? taskById.get(marker[1]) : undefined;
    if (!t || typeof t.fm.contract !== 'string' || t.fm.contract === '') continue;
    const declared = codesByContract.get(t.fm.contract);
    if (declared === undefined) continue; // 契约不存在/不可解析已由 V-TASK-03 / V-CON-01 报
    const seen = new Set();
    for (const { m, line } of scanSource(f.text, BIZ_THROW_SOURCE)) {
      const name = m[1];
      const code = errorEnum.get(name);
      if (code === undefined || seen.has(name)) continue;
      seen.add(name);
      if (declared.has(code)) continue;
      if (declaredAnywhere.has(code)) {
        errors.push(entry(
          'V-CODE-03',
          `第 ${line} 行抛出契约未声明的错误码 ${code}（ErrorCode.${name}）——契约 ${t.fm.contract} `
            + `的 errors[] 里没有它，而项目别处的契约声明过该码（即它确实是业务码）：`
            + `请先在契约对应端点补 { code: ${code}, msg: … } 再写代码（§9.5 契约唯一真源）；`
            + `若本文件其实属于别的模块，则是 @vima 标注错了，改标注即可`,
          f.rel,
        ));
      } else {
        warnings.push(entry(
          'V-CODE-03',
          `第 ${line} 行抛出错误码 ${code}（ErrorCode.${name}），而它在**全部契约**里都没被声明过`
            + `——两种可能，需人裁定：①它是由底座统一拦截的框架码（如认证失效），`
            + `那么按约定本就不进业务契约的 errors[]，此条可忽略；`
            + `②它是业务码而整个项目都漏了声明，那么每个会抛它的端点都要补 `
            + `{ code: ${code}, msg: … }。vima 无法替你区分——错误码的分类约定`
            + `不是 vima 的资产，判定权在项目`,
          f.rel,
        ));
      }
...
/** 后端权限点：`@PreAuthorize("@perm.has('system:user:add')")`（骨架 PermChecker 约定）。 */
const PERM_HAS_SOURCE = String.raw`@perm\s*\.\s*has\(\s*'([^']+)'\s*\)`;
/** 前端权限点：`v-auth="'x'"` / `v-auth="['x', 'y']"`（骨架 auth.ts 指令约定）。 */
const V_AUTH_SOURCE = String.raw`v-auth\s*=\s*(["'])([\s\S]*?)\1`;
const QUOTED_RE = /['"]([^'"]+)['"]/g;

/**
 * V-CODE-04 权限点三面对账（A42 D-A42-02）：
 * spec `vima:menus[].features[].perms` ↔ 后端 `@perm.has('X')` ↔ 前端 `v-auth="X"`。
 * 缺后端 ⇒ **error**（接口不设防，越权风险）；缺前端 ⇒ **warn**（入口不设防，
 * 点下去才吃 403——是体验缺陷不是安全缺陷，故不等级齐平）。
 *
 * **不含菜单种子**（A42 明写：种子是项目自建产物，vima 无法通用定位；三面已足够）。
 *
 * 零假阳性闸门：**某一面没有任何带标注的业务代码 ⇒ 该面整体不判**。规格期（代码尚未
 * 存在）与「本端还没开工」都落在这道闸门里——一条「报缺失」的规则若不设此闸，
 * 会在 PLANNING 阶段把每个权限点都报一遍，正是 A42 要治的那种永远消不掉的告警。
 */
function checkPerms(sources, spec, roster, errors, warnings) {
  if (!spec) return;
  const declared = []; // { perm, menuId, feature, app }
  for (const menu of Array.isArray(spec.menus) ? spec.menus : []) {
    if (!menu || typeof menu !== 'object') continue;
    for (const feat of Array.isArray(menu.features) ? menu.features : []) {
      if (!feat || typeof feat !== 'object' || feat.perms === undefined) continue;
      const list = Array.isArray(feat.perms) ? feat.perms : [feat.perms];
      for (const perm of list) {
        if (typeof perm !== 'string' || perm.trim() === '') continue;
        declared.push({
          perm: perm.trim(),
          menuId: typeof menu.id === 'string' ? menu.id : '(未命名菜单)',
          feature: typeof feat.name === 'string' ? feat.name : '(未命名功能点)',
          app: appOf(menu, roster),
        });
      }
    }
  }
  if (declared.length === 0) return; // 未声明 perms 的项目完全不触发（存量零影响）

  const beHits = new Set();
  for (const f of sources.backend) {
    for (const { m } of scanSource(f.text, PERM_HAS_SOURCE)) beHits.add(m[1]);
  }
  const feHits = new Map(); // appId → Set<perm>；只登记「该端确有带标注的业务代码」的端
  for (const { app, files } of sources.frontend) {
    if (files.length === 0) continue;
    const hits = new Set();
    for (const f of files) {
      for (const { m } of scanSource(f.text, V_AUTH_SOURCE)) {
        for (const q of m[2].matchAll(QUOTED_RE)) hits.add(q[1]);
      }
    }
    feHits.set(app.id, hits);
  }

  for (const d of declared) {
    const where = `权限点 "${d.perm}"（${d.menuId} 功能点「${d.feature}」）`;
    if (sources.backend.length > 0 && !beHits.has(d.perm)) {
      errors.push(entry(
        'V-CODE-04',
        `${where}在后端无任何 @PreAuthorize("@perm.has('${d.perm}')")——接口不设防，`
          + '任何登录用户都能调用（越权风险）：请在实现该功能点的 Controller 方法上补注解',
        `${sources.backendDir}/src`,
      ));
    }
    // 端归属取不到（多端项目菜单缺 app，V-SPEC-13 已报 error）⇒ 前端面不判，不猜哪个端
    const fe = sources.frontend.find((x) => x.app.id === d.app);
    if (fe && feHits.has(fe.app.id) && !feHits.get(fe.app.id).has(d.perm)) {
      warnings.push(entry(
        'V-CODE-04',
        `${where}在前端 ${fe.subdir} 无任何 v-auth="${d.perm}"——入口不设防：`
          + '无权用户看得见按钮、点下去才吃 403；请在对应按钮/入口补 v-auth（权限码须与后端一字不差）',
        fe.subdir,
      ));
    }
  }
...
/** 代码里的编号引用：`data-page="PAGE-03"`、`<!-- MODAL-02 -->`、`// 见 RULE-35`。 */
const SPEC_ID_SOURCE = String.raw`\b(PAGE|MODAL|RULE|NG)-(\d+)\b`;

/**
 * V-SRC-02（error）编号引用校验（A42 D-A42-02）：业务代码里的
 * `PAGE-\d+` / `MODAL-\d+` / `RULE-\d+` / `NG-\d+` 必须存在于 spec 对应章节。
 *
 * 为什么是 error 而不是 warn：**A22 字段级机检、A34 保真验收、converge 点位对账全都以
 * `data-page` 为锚**——锚点指向不存在的页面时这些机制不报错，而是静默地什么都不检。
 * 它同时拦住 builder 给自己发豁免（实证：`NG-16` 被当作「微信登录留桩」的豁免依据，
 * 而 spec 止于 NG-15）。
 *
 * 存在性集合**只取 spec 的结构化数据块**（pages / modals / rules / non-goals），不看正文：
 * spec 里声明为「空号/保留不复用」的编号只存在于正文，因而天然算「不存在」——
 * 这正是要的判据（空号被引用 = 悬空锚点）。
 * 注释一并扫（`skipComments: false`）：`// 参见 PAGE-41` 与 `data-page="PAGE-41"`
 * 同为悬空引用，且注释里的编号正是 builder 自发豁免的常见落点。
 */
function checkSpecIdRefs(sources, spec, errors) {
  if (!spec) return;
  const known = { PAGE: new Set(), MODAL: new Set(), RULE: new Set(), NG: new Set() };
  for (const [id, page] of spec.pages) {
    known.PAGE.add(id);
    for (const mo of Array.isArray(page?.modals) ? page.modals : []) {
      if (mo && typeof mo === 'object' && typeof mo.id === 'string') known.MODAL.add(mo.id);
    }
  }
  for (const r of Array.isArray(spec.rules) ? spec.rules : []) {
    if (r && typeof r === 'object' && typeof r.id === 'string') known.RULE.add(r.id);
  }
  for (const n of Array.isArray(spec.nonGoals) ? spec.nonGoals : []) {
    if (n && typeof n === 'object' && typeof n.id === 'string') known.NG.add(n.id);
  }
  const CHAPTER = { PAGE: 'spec 第三章 页面清单', MODAL: 'spec 页面块的 modals', RULE: 'spec 第五章 vima:rules', NG: 'spec 第九章 vima:non-goals' };

  const files = [...sources.frontend.flatMap((x) => x.files), ...sources.backend];
  for (const f of files) {
    const seen = new Set();
    for (const { m, line } of scanSource(f.text, SPEC_ID_SOURCE, { skipComments: false })) {
      const id = m[0];
      if (seen.has(id) || known[m[1]].has(id)) continue;
      seen.add(id);
      errors.push(entry(
        'V-SRC-02',
        `第 ${line} 行引用了 ${CHAPTER[m[1]]} 中不存在的编号 ${id}——`
          + 'data-page/data-block 与注释里的编号是 A22 字段级机检、A34 保真验收、converge '
          + '点位对账的共同锚点，锚点悬空时这些机检不是报错而是**静默地什么都不检**；'
          + `请改为真实编号，或先在 spec 里补 ${id}（spec 中声明为空号/不复用的编号不算存在）`,
        f.rel,
      ));
    }
  }
...
// ---------------------------------------------------------------------------
// 覆盖矩阵（V-COV-01 error / V-COV-02 warn）
// ---------------------------------------------------------------------------

/** 未承接标记：render-matrix 对「尚无任务承接」渲染成以此开头的破折号串（A44 D-A44-03）。 */
const UNCOVERED_PREFIX = '—';

/**
 * V-COV-01（error）结构 + V-COV-02（warn）未承接行点名。
 *
 * **逐表校验**（A44 D-A44-02）：此前把全文件所有 `|…|` 行当成一张表，
 * 首个非分隔行当表头、其余一律是数据行——文件里出现第二张表时整片误报列数不一致。
 * 切分改由 `splitMarkdownTables` 统一承担（validate 与 retro 共用一份实现）。
 */
async function checkCoverage(root, errors, warnings, multi = false) {
  ...
  const tables = splitMarkdownTables(text).filter((t) => t.header.length >= 3);
  if (tables.length === 0) {
    errors.push(entry('V-COV-01', '覆盖矩阵缺少 ≥3 列的对齐表格', COVERAGE_REL));
    return;
  }
  // A16：多端项目首列须为「端」（产物由 vima render-matrix 生成，此处防手改漂移）。
  // 只查**首表**（页面承接表）——第二张「业务规则承接」表按规则维度组织，无端列。
  const first = tables[0];
  if (multi && first.header[0] !== '端') {
    errors.push(entry('V-COV-01', `多端项目覆盖矩阵首列须为「端」（当前首列：「${first.header[0]}」）——请重跑 vima render-matrix`, COVERAGE_REL));
  }
  ...
  for (const table of tables) {
    const headerCols = table.header.length;
    const label = table.header.join('/');
    for (let i = 0; i < table.rows.length; i++) {
      const cells = table.rows[i];
      if (cells.length !== headerCols) {
        errors.push(entry('V-COV-01', `覆盖矩阵「${label}」表第 ${i + 1} 数据行列数为 ${cells.length}，须与表头 ${headerCols} 列一致`, COVERAGE_REL));
      }
      for (let j = 0; j < cells.length; j++) {
        if (cells[j] === '' || /\bTODO\b/i.test(cells[j])) {
          errors.push(entry('V-COV-01', `覆盖矩阵「${label}」表第 ${i + 1} 数据行第 ${j + 1} 列存在缺口（空单元格或 TODO）`, COVERAGE_REL));
        }
      }
      // V-COV-02：末列（承接任务）为未承接标记时逐条点名。
      // 恒 warn：PLANNING 期尚未派任务是正常中间态，判 error 会阻断开工（C4 不阻塞）。
      const last = cells[cells.length - 1] ?? '';
      if (cells.length === headerCols && last.startsWith(UNCOVERED_PREFIX)) {
        warnings.push(entry(
          'V-COV-02',
          `覆盖矩阵「${label}」表第 ${i + 1} 行尚无任务承接：${cells[0]}——`
            + '补任务或在 spec 第八章记录「本期不承接」的决策，别让它静默留在矩阵里',
          COVERAGE_REL,
        ));
      }
    }
...
export async function validateProject(root, { artifact, cliRoot, profile } = {}) {
  ...
  // ── 覆盖矩阵：V-COV-01（error 结构）+ V-COV-02（warn 未承接行）──
  if (groups.coverage) {
    await checkCoverage(root, errors, warnings, roster.multi);
  }
  ...
  if (groups.code) {
    ...
    const sources = await collectMarkedSources(root, roster);
    checkFrontendCode(sources, contractKeys, errors, consumersByKey);
    checkBackendCode(sources, contractKeys, errors);
    // A42 D-A42-02 接缝对账（同一份扫描结果，同一作用域）
    checkErrorCodes(sources, tasks, contracts, await loadErrorCodeEnum(root, sources.backendDir), errors, warnings);
    checkPerms(sources, spec, roster, errors, warnings);
    checkSpecIdRefs(sources, spec, errors);
  }
