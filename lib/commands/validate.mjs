  ...
  for (const [id, page] of pages) {
    ...
    const collect = (words, at) => {
      for (const word of words) {
        if (!LAYOUT_VOCAB.has(word)) {
          errors.push(entry('V-SPEC-12', `页面 ${id} 的 ${at} 含非法词 "${word}"（词表：${[...LAYOUT_VOCAB].join('|')}）`, SPEC_REL));
        }
...
async function checkTasks(root, tasks, spec, contracts, errors, warnings) {
...
export async function validateProject(root, { artifact } = {}) {
  ...
  // ── spec 组：V-SPEC-01…11 + V-DEC-01 + V-YAML-01 ──
  if (groups.spec && spec) {
  ...
  // ── 契约组：V-CON-01…06 + V-YAML-01 + V-SRC-01 ──
  if (groups.contracts) {
  ...
  // ── 任务组：V-TASK-01…09 ──
  if (groups.tasks) {
    await checkTasks(root, tasks, spec, contracts, errors, warnings);
  }
