...
/** 读一份 Agent 报告；缺失或坏 JSON 一律 null（「无通过证据」，不区分成因）。 */
async function readReport(abs) {
  try {
...
/**
 * 校验 §6.9 的可选 `commands` 字段形状（A43 D-A43-01）。
 *
 * 与 post-write hook 的 `reportEventOf` 同口径：`undefined` → 视为「未声明」返回 `[]`；
 * 非数组或任一条目不合形 → 返回 `null`（协议没对齐，宁缺勿假，不做部分采信）。
 * 合形定义：`cmd` 为非空字符串，`exitCode` 为整数。
 */
export function wellFormedCommands(value) {
  ...
  for (const c of value) {
    ...
    out.push({ cmd: c.cmd, exitCode: c.exitCode });
  }
  return out;
}
...
async function checkImplemented(root, tasks, ctx) {
  ...
  for (const t of impl.filter((x) => x.fm.layer === 'business' && x.fm.status === 'done')) {
    ...
    // 缺失或损坏都算无通过证据（readReport 已把两种成因归一）
    const rep = await readReport(path.join(root, rel));
    if (rep === null || rep.result !== 'pass') noPass.push(t.id);
  }
  if (noPass.length > 0) {
...
async function checkPipelineGreen(root, tasks) {
  ...
  if (undone.length > 0) missing.push(`pipeline 任务未全部 done：${undone.join('、')}`);

  // A43 D-A43-02：与 implemented 级同口径读报告，并额外要求命令留痕。
  // 只对 done 的任务取证——未 done 的缺口已由上一条列出，重复报同一件事是噪声。
  const noPass = [];
  const noCommands = [];
  const redCommands = [];
  let commandTotal = 0;
  for (const t of pipeline.filter((x) => x.fm.status === 'done')) {
    const rel = `.vima/reports/${t.id}-verifier.json`;
    const rep = await readReport(path.join(root, rel));
    if (rep === null || rep.result !== 'pass') { noPass.push(t.id); continue; }
    const cmds = wellFormedCommands(rep.commands);
    if (cmds === null || cmds.length === 0) { noCommands.push(t.id); continue; }
    commandTotal += cmds.length;
    const red = cmds.filter((c) => c.exitCode !== 0);
    if (red.length > 0) {
      redCommands.push(`${t.id}（${red.map((c) => `${c.cmd} → exit ${c.exitCode}`).join('；')}）`);
    }
  }
  if (noPass.length > 0) {
    missing.push(`缺 Verifier 通过报告（<taskId>-verifier.json result=pass）：${noPass.join('、')}`);
  }
  if (noCommands.length > 0) {
    missing.push(
      `缺命令留痕（报告 commands 非空且每条形如 {cmd, exitCode}）：${noCommands.join('、')}`
        + '——pipeline 任务的职责就是跑命令，没有命令与退出码就没有「流水线绿」的证据',
    );
  }
  if (redCommands.length > 0) {
    missing.push(`命令退出码非 0：${redCommands.join('；')}`);
  }
  if (missing.length > 0) return { satisfied: false, evidence: [], missing };
  ...
  return {
    satisfied: true,
    evidence: [
      `pipeline 任务 ${pipeline.length} 个（${pipeline.map((t) => t.id).join('、')}）全部 done`,
      'pipeline 任务的 Verifier 报告全部 result=pass（.vima/reports/<taskId>-verifier.json）',
      `报告留痕命令 ${commandTotal} 条，退出码全部为 0`,
    ],
    missing: [],
  };
}
...
export async function run(argv, ctx) {
  ...
  const levels = [
    ...
    { level: 'pipeline-green', ...(await checkPipelineGreen(root, tasks)) },
  ];
