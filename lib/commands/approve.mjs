...
export async function run(argv, ctx) {
  ...
  if (designScoped) {
    ...
    // 同口径恒不阻断（A45 D-A45-03）：方向裁定属〔L5·人审〕，CLI 判不出敲回车的是谁，
    // 但「这一票没有确证」必须在开工前的最后一刻被念出来，而不是只留在 selection.md 里。
    if (gate.directionSelectors.length > 0) {
      out(`⚠️ ${gate.directionSelectors.length} 个端的方向裁定人不是用户（不阻断，由你复核）：`);
      for (const s of gate.directionSelectors) {
        out(`   ${s.app}：${s.selectedBy === 'agent' ? 'Agent 代选' : '未记录裁定人'}`
          + `${s.reason === null ? '' : ` —— ${s.reason}`}`);
      }
      out('   若这不是你选的方向，先 vima design invalidate --app <id>，别让它带进 DEVELOPING。');
    }
  }
