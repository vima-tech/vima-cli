...
export function buildView(raw, now) {
  const { lifecycle, tasks, events, batchPlan, dataIssues, reopenedAt } = raw;
  ...
  const trust = trustSignals({ tiers, groups, lifecycle, batchPlan });
...
export function renderTable(v, projectName) {
  ...
  for (const layer of ['shared', 'pipeline']) {
    const c = v.groups.byLayer[layer];
    if (c.total === 0) continue;
    L.push(gline({ shared: '共享层', pipeline: '流水线' }[layer], c));
  }
  ...
  L.push(
    v.activity.lastEventAt === null
      ? '活动   journal 无事件——尚未产生任何过程轨迹'
      : `活动   最近一条轨迹事件距今 ${fmtDuration(v.activity.idleMs)}（共 ${v.activity.events} 条）`,
  );
  L.push('');
...
export async function run(argv, ctx) {
  ...
  // `--line` 恒 exit 0 且恒输出单行（D-A37-04）：statusLine 的宿主拿到非零退出码
