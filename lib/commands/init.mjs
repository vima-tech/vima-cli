...
//              docs/design-language.md（← designLanguage 骨架，A30 风格真源：缺省取向 + 取向轴定档
//              + Stage A 模式库容器，三者都属项目自己的决定，故 userOwned）
...
export async function buildInstallPlan(root, template, vars, { minimal = false, skipScan = false, roster = null } = {}) {
  ...
  // managed：.claude/ 工作环境（settings + skills + commands + agents + hooks）
  ...
  if (!minimal) {
    ...
    // userOwned：设计语言骨架（A30）——缺省风格取向 + 取向轴定档位 + Stage A 模式库容器。
    // 与 spec 同口径（变量替换 + update 永不覆盖）：定档与模式库条目是项目自己的决定。
    if (planning.designLanguage) {
      userOwned.push({
        rel: 'docs/design-language.md',
        content: renderVars(await asset(planning.designLanguage), vars),
      });
    }
    // userOwned：交互语言骨架（A34 D-A34-24）——Stage A 的第二个产出。
    // design-language.md 管版面语言，本份管交互语言；两者一起进 designSystemFrozen 摘要。
    if (planning.interactionLanguage) {
      userOwned.push({
        rel: 'docs/interaction-language.md',
        content: renderVars(await asset(planning.interactionLanguage), vars),
      });
    }
    // managed：规划资产
...
export async function run(argv, ctx) {
  ...
  const structure = minimal
    ? [
        ...
        '  .claude/                     # skills / 兼容命令 / 子代理 / hooks / settings',
    ...
    : [
        ...
        '    spec.md                    # 规格骨架（九章，待填充）',
        '    design-language.md         # 设计语言（风格取向定档 + 版面模式库）',
        '    interaction-language.md    # 交互语言（交互决策条目 + 各自的执行者）',
        '    raw/                       # 存放原始文档',
        ...
        '  .claude/',
        '    skills/                    # 正式工作流入口（/go，可自然语言触发）',
        '    commands/                  # 兼容斜杠命令与工作流正文',
        '    agents/                    # 子代理角色模板',
  ...
  process.stdout.write(
    [
      ...
      '  2. 运行 vima go 从项目根启动 Claude Code 并进入 /go',
      '  3. 通过自然语言描述需求，让 Agent 按 docs/planning-guide.md 整理文档',
      '',
    ].join('\n'),
  );
  warnClaudeSessionAssets(root);
  return EXIT.OK;
}
