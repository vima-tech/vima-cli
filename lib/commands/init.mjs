...
//              —— 已存在则跳过并提示「保留用户版本」，upgrade 永不覆盖；
//   managed  ：docs/planning-guide.md、docs/coding-standards.md、docs/planning-validation/*、docs/contracts/_example.md、
...
//              全量 sha256 记入 .vima/manifest.json 供 vima upgrade 三方比较。
...
import { defaultLifecycle, saveLifecycle } from '../model/lifecycle.mjs';
import { loadManifest, saveManifest } from '../model/manifest.mjs';
import { parseOpts, projectPkgOf, renderVars, readCliVersion } from './create.mjs';
...
// 契约 §6.4 userOwned 列表（原样落盘，upgrade 永不触碰）
const USER_OWNED = ['CLAUDE.md', 'docs/spec.md', 'docs/contracts/', 'docs/tasks/', 'docs/raw/', 'docs/coverage-matrix.md'];

/**
 * 构建安装计划（init 与 upgrade 共用同一份「managed 文件 → 模板源」映射）。
 * @returns {{ managed: Array<{rel,content,mode?}>, userOwned: Array<{rel,content}> }}
 */
export async function buildInstallPlan(root, template, vars, { minimal = false, skipScan = false } = {}) {
  const managed = [];
  const userOwned = [];
  ...
  if (!minimal) {
    ...
    if (!skipScan) {
      const uiDocsDir = path.join(template.dir, 'ui-docs');
...
/** 落盘一个 managed 计划项（原子写 + 可执行位），upgrade 复用。 */
...
export async function run(argv, ctx) {
  ...
  if (initialized && !values.force) {
    throw precondition('ALREADY_INIT', '项目已初始化。升级请使用 vima upgrade，强制重建请加 --force');
  }
  ...
  // 5. docs/lifecycle.json：BOOTSTRAP completedAt=now、PLANNING enteredAt=now（契约 §6.2）
  ...
  for (const rel of skipped) {
    process.stdout.write(`⚠️ ${rel} 已存在，保留用户版本（未覆盖）\n`);
  }
