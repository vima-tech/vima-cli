...
// 是**先有后台、后要小程序端**。端册只能由 create 一次写死的话，存量项目要加第二个前端就只能
...
import { projectPkgOf, renderVars, templateAppEntry, resolveScaffoldEntries } from './create.mjs';
...
export async function run(argv, ctx) {
  ...
  if (sub === 'list') {
    if (argv.length > 1) throw usageError(`多余的位置参数 "${argv[1]}"`);
  ...
  vars.projectPkg = projectPkgOf(vars.projectName);
