...
//   ⑬任务 updatedAt 未来时间戳（A38）⑭Claude Code 资产可达性（A40：hook 锚定/会话根/采集链路）
...
import { loadLifecycle, PHASES } from '../model/lifecycle.mjs';
...
// A34 D-A34-30：阶段序列是横切概念，从 lifecycle 模型取单一真源（见上方 import）——
// 此前这里是本地字面量，新增 DESIGNING 后会把合法阶段判成不合法（状态机分叉）。
...
const CLAUDE_DIR_REQUIRED = [
  ...
  '.claude/agents/vima-designer.md',
  '.claude/agents/vima-design-reviewer.md',
  '.claude/agents/vima-experience-verifier.md',
  '.claude/hooks/guard-shared.mjs',
  '.claude/hooks/post-write.mjs',
  '.claude/hooks/go-continue.mjs', // A18：/go 批间续跑器（Stop hook）
  '.claude/hooks/go-autostart.mjs', // A39：DEVELOPING 期自动开工（SessionStart hook）
];
const CLAUDE_SKILLS = ['go', 'check', 'design', 'vima'];
const HOOK_FILES = [
  '.claude/hooks/guard-shared.mjs',
  '.claude/hooks/post-write.mjs',
  '.claude/hooks/go-continue.mjs',
  '.claude/hooks/go-autostart.mjs',
];
...
/**
 * 任务 `updatedAt` 不得晚于当前时钟（A38 D-A38-03，L3「执行忠实」的唯一机检）。
 *
 * **判据为什么零假阳性**：内核写 `updatedAt` 一律用 `new Date().toISOString()`，
 * 不可能产出未来时刻。出现未来值只有一个解释——**这行是 Agent 手写的**，
 * 即有人绕过 CLI 直接改了任务 frontmatter。sustain-v4 实证里这条会命中 6 次
 * （最远 `19:30Z` vs 当时真实 `07:51Z`）。
 *
 * `SKEW_MS` 容差留给机器时钟不准与跨机协作；超出容差才算数。
 * 本项**只做这一条**：其余「是否手工模拟了 CLI」大多不可判定（谁写的文件无从追溯），
 * 而 taskStats 打架已由 ④ 覆盖、三档进度落差已由 `vima status` 呈现，不重复。
 * 报 error 但**不进 validate、不拦 /go**——成因可能只是时钟不准，拦下开发得不偿失。
 */
const FUTURE_SKEW_MS = 5 * 60 * 1000;

function checkTaskTimestamps(tasks, tasksError, now) {
  const name = '⑬ 任务 updatedAt 未来时间戳（绕过内核的痕迹）';
  if (tasksError) return result('task-timestamps', name, 'warn', ['任务文件不可读，跳过']);
  if (tasks.length === 0) return result('task-timestamps', name, 'ok', ['无任务文件']);

  const future = [];
  for (const t of tasks) {
    const ts = Date.parse(t.fm?.updatedAt ?? '');
    if (Number.isFinite(ts) && ts - now > FUTURE_SKEW_MS) {
      future.push(`${t.file}（updatedAt ${t.fm.updatedAt}）`);
    }
  }
  if (future.length === 0) {
    return result('task-timestamps', name, 'ok', [`${tasks.length} 个任务的 updatedAt 均不晚于当前时钟`]);
  }
  return result('task-timestamps', name, 'error', [
    `${future.length} 个任务的 updatedAt 晚于当前时钟——内核只写真实时间，未来值意味着 frontmatter 被手改：`,
    ...future,
    '状态改动请走完成动作 + vima sync，不要手写 frontmatter；若确系本机时钟不准，请先校时。',
  ]);
}
...
async function checkClaudeDir(root) {
  const name = '⑥ .claude 配置完整性（4 skills + 3 工作流正文 + 6 角色 + 4 hooks）';
  const missing = [];
  for (const rel of CLAUDE_DIR_REQUIRED) {
    if (!(await fileExists(path.join(root, rel)))) missing.push(rel);
  }
  if (missing.length > 0) {
    return result('claude-dir', name, 'error', [
      `缺失：${missing.join('、')}——运行 vima update 修复（尚未初始化则运行 vima init）`,
    ]);
  }
  const invalid = [];
  for (const skill of CLAUDE_SKILLS) {
    ...
    try {
      ...
      const meta = fm === null ? null : parseYaml(fm, { path: rel });
      if (!meta || typeof meta.description !== 'string' || meta.description.trim() === '') {
        invalid.push(`${rel} 缺有效 description`);
      }
      const disabled = meta?.['disable-model-invocation'];
      if (
        disabled === true
        || disabled === 1
        || (typeof disabled === 'string' && ['true', 'yes', 'on'].includes(disabled.toLowerCase()))
      ) {
        invalid.push(`${rel} 禁用了模型触发`);
      }
      if (!body.includes('${CLAUDE_SKILL_DIR}') || !body.includes('${CLAUDE_PROJECT_DIR}')) {
        invalid.push(`${rel} 缺项目根校验变量`);
      }
    } catch (err) {
      invalid.push(`${rel} frontmatter 无效：${err.message}`);
    }
  }
  if (invalid.length > 0) {
    return result('claude-dir', name, 'error', [
      `${invalid.join('；')}——运行 vima update 修复`,
    ]);
  }
...
// ── ⑭ Claude Code 资产可达性（A40）─────────────────────────────────────────
/**
 * 前六项查的都是「资产在不在、能不能执行」，**没有一项查它到底有没有生效**。
 * 实战失效（sustain-v4，2026-08-15）：⑥⑦⑧ 全绿、doctor 报「体检通过」，而项目的
 * 4 个 hook、6 个子代理、4 个 skill 在那条会话里**一个都没注册**——会话开在项目的
 * 父目录，Claude Code 只从会话项目根加载 `.claude/settings.json`。后果是 guard 不设防、
 * post-write 巡检从未运行、journal 零 report 事件，`vima status` 三档恒 0/0/0。
 * A37 的 `D-A37-02` 把「差值成因」的判定明确划给 doctor，本项即是兑现。
 *
 * 三条判据（都确定性，不猜）：
 *   a. hook 命令是否锚定项目根——`node .claude/hooks/x.mjs` 按 **cwd** 解析，
 *      Agent `cd apps/admin` 之后就找不到脚本，且失败是静默的；
 *   b. 会话项目根是否就是本项目——`CLAUDE_PROJECT_DIR` 存在且指向别处时，本项目的
 *      `.claude/` 整体未注册（这一条只在 Claude 会话里跑 doctor 时才有信号）；
 *   c. 采集链路活性——报告文件已落盘却没有对应 journal report 事件 = hook 没接上。
 */
async function checkClaudeReachability(root, lifecycle) {
  const name = '⑭ Claude Code 资产可达性（hook 锚定 / 会话根 / 采集链路）';
  const errors = [];
  const warns = [];
  const oks = [];
  ...
  if (await fileExists(settingsPath)) {
    ...
    try {
      ...
      warns.push(`${settingsRel} 不可解析（${err.message}），跳过锚定检查`);
    }
    const unanchored = [];
    for (const entries of Object.values(settings?.hooks ?? {})) {
      for (const group of Array.isArray(entries) ? entries : []) {
        for (const h of group?.hooks ?? []) {
          ...
          if (!cmd.includes('.claude/hooks/')) continue;
          // 判「锚定」看**引用 .claude/hooks/ 的那个词元本身**：带 $CLAUDE_PROJECT_DIR，
          // 或去引号后以 / 开头（绝对路径）。看整条命令会被解释器路径（node 的 /usr/…）
          // 或无空格的直执行形式（"/abs/.claude/hooks/x.mjs"）带偏。
          const token = cmd.split(/\s+/).find((w) => w.includes('.claude/hooks/')) ?? '';
          const bare = token.replace(/^["']|["']$/g, '');
          if (!bare.includes('$CLAUDE_PROJECT_DIR') && !bare.includes('${CLAUDE_PROJECT_DIR}')
              && !bare.startsWith('/')) {
            unanchored.push(cmd);
          }
        }
    ...
    if (unanchored.length > 0) {
      errors.push(
        `${unanchored.length} 条 hook 命令未锚定项目根（按 cwd 解析，Agent 切到子目录后静默失效）`
        + '——运行 vima update 迁移为 "$CLAUDE_PROJECT_DIR/.claude/hooks/…"',
      );
    } else if (settings) {
      oks.push('hook 命令均锚定 $CLAUDE_PROJECT_DIR');
    }
  }
  ...
  if (sessionDir) {
    ...
    if (sessionRoot !== path.resolve(root)) {
      errors.push(
        `本会话的项目根是 ${sessionRoot}，不是本项目 ${root}——`
        + '本项目的 .claude（hooks/子代理/skills）在本会话中全部未注册，'
        + `guard 不设防、巡检不运行、journal 无轨迹。请在项目根重开会话（cd ${root} && claude）或用 vima go`,
      );
    } else {
      oks.push('会话项目根与本项目一致');
    }
  }

  // ── c. 采集链路活性 ──
  const reportsDir = path.join(root, '.vima', 'reports');
  let reportTasks = 0;
  try {
    for (const f of await readdir(reportsDir)) {
      if (/-(builder|verifier)\.json$/.test(f)) reportTasks += 1;
    }
  } catch { /* 目录不存在 = 尚未产生报告 */ }
  let journalReports = 0;
  try {
    const raw = await readFile(path.join(reportsDir, 'journal.jsonl'), 'utf8');
    for (const line of raw.split('\n')) {
      if (line.trim() === '') continue;
      try {
        if (JSON.parse(line)?.kind === 'report') journalReports += 1;
      } catch { /* 坏行忽略：journal 是 append-only，容忍半行 */ }
    }
  } catch { /* 无 journal */ }
  if (reportTasks > 0 && journalReports === 0) {
    errors.push(
      `${reportTasks} 份任务报告已落盘，但 journal 里 0 条 report 事件——`
      + 'post-write hook 未生效，vima status 的「有轨迹/已验收」两档会恒为 0',
    );
  } else if (reportTasks > 0) {
    oks.push(`采集链路通畅（${reportTasks} 份报告 / ${journalReports} 条 journal 事件）`);
  } else if (lifecycle?.currentPhase === 'DEVELOPING') {
    warns.push('开发期尚无任何任务报告落盘——若已在写代码，说明没走 /go 的 Builder→Verifier 链路');
  }

  if (errors.length > 0) return result('claude-reach', name, 'error', errors.concat(warns));
  if (warns.length > 0) return result('claude-reach', name, 'warn', warns.concat(oks));
  return result('claude-reach', name, 'ok', oks.length > 0 ? oks : ['无 Claude Code 资产，跳过']);
}
...
async function checkAppsRoster(root, ctx, lifecycle) {
  ...
  // DESIGNING 与 PLANNING 同属设计先行合法态（骨架可后补）；DEVELOPING 起才阻断。
...
export async function run(argv, ctx) {
  ...
  if (vimaProject) {
    ...
    checks.push(checkTaskTimestamps(tasks, tasksError, Date.now())); // A38 D-A38-03
    ...
    checks.push(await checkClaudeReachability(root, lifecycle)); // A40 资产可达性
