...
//   ⑥.claude 完整性 ⑦hooks 可执行位 ⑧manifest 校验和 ⑨对齐产物漂移
...
const HOOK_FILES = ['.claude/hooks/guard-shared.mjs', '.claude/hooks/post-write.mjs'];
...
async function checkClaudeDir(root) {
  const name = '⑥ .claude 配置完整性（settings + 2 命令 + 3 角色 + 2 hooks）';
  const missing = [];
...
async function checkManifest(root) {
  ...
  const statuses = [];
  if (missing.length > 0) {
    messages.push(`受管文件缺失：${missing.join('、')}——运行 vima upgrade 修复`);
    statuses.push('error');
  }
