...
import { saveManifest } from '../model/manifest.mjs';
...
/** 统一 parseArgs 封装：非法/未知选项转 usageError（exit 3）。C1 三命令共用。 */
...
/** projectAbbr：项目名去掉非字母数字后取前 2 字符大写（侧边栏 Logo 缩写等场景）；空则回退 VM。 */
export function projectAbbrOf(name) {
  const abbr = name.replace(/[^a-zA-Z0-9]/g, '').slice(0, 2).toUpperCase();
  return abbr || 'VM';
}

/** 模板变量替换：内容与相对路径中的 {{projectName}}/{{projectPkg}}/{{projectAbbr}}/{{createdAt}}。 */
export function renderVars(text, vars) {
  return text.replace(/\{\{(projectName|projectPkg|projectAbbr|createdAt)\}\}/g, (_, k) => String(vars[k] ?? ''));
}
...
/**
 * 拷贝 scaffold 到目标目录：scaffold 字段为 { 目标位置: 相对目录 }，目标位置 "." 或 ""
 * 表示项目根（设计 §15：前端 src/ 在根，后端在 backend/）；缺省（preview 模板）时
 * 整个 scaffold/ 落到项目根。相对路径一律做变量替换；内容仅对文本文件替换，
 * 二进制（.png/.woff2 等）原字节透传；源文件可执行位保留。
 */
async function copyScaffold(template, target, vars) {
  const entries =
    template.scaffold && typeof template.scaffold === 'object'
      ...
      : [['', 'scaffold']];
  let count = 0;
  for (const [destKeyRaw, srcRel] of entries) {
...
export async function run(argv, ctx) {
  ...
  const templateId =
    !values.interactive && values.template ? values.template : await chooseTemplate(ctx.cliRoot);
  ...
  const vars = { projectName: name, projectPkg: projectPkgOf(name), projectAbbr: projectAbbrOf(name), createdAt };
  const fileCount = await copyScaffold(template, target, vars);
  process.stdout.write(`已生成骨架文件 ${fileCount} 个（模板：${template.name} ${template.id}）\n`);
  ...
  if (!values['no-git']) {
    ...
    if (r.error || r.status !== 0) {
      process.stdout.write('⚠️ git 不可用或初始化失败，已跳过 git init（可稍后手动执行）\n');
    } else {
      process.stdout.write('✅ 已初始化 Git 仓库\n');
    }
  }

  // npm install（--no-install 跳过；在项目根执行——前端 package.json 在根，设计 §15；失败 → 警告不中止）
  if (!values['no-install']) {
    if (await fileExists(path.join(target, 'package.json'))) {
      process.stdout.write('正在安装前端依赖（npm install，可能需要几分钟）…\n');
      const r = spawnSync('npm', ['install'], { cwd: target, encoding: 'utf8' });
      if (r.error || r.status !== 0) {
        process.stdout.write('⚠️ npm install 失败，已跳过（可稍后手动执行 npm install）\n');
      } else {
        process.stdout.write('✅ 前端依赖安装完成\n');
      }
    }
  }
