...
/** 模板变量替换：内容与相对路径中的 {{projectName}}/{{projectPkg}}/{{createdAt}}。 */
export function renderVars(text, vars) {
  return text.replace(/\{\{(projectName|projectPkg|createdAt)\}\}/g, (_, k) => String(vars[k] ?? ''));
}
...
export async function run(argv, ctx) {
  ...
  if (!values['no-install']) {
    if (await fileExists(path.join(target, 'package.json'))) {
      ...
      if (r.error || r.status !== 0) {
        process.stderr.write('⚠️ npm install 失败，已跳过（可稍后手动执行 npm install）\n');
      } else {
