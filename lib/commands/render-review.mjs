...
import { atomicWriteFile, driftOf, fileExists } from '../util/fs.mjs';
...
export async function run(argv, ctx) {
  ...
  const model = {
    ...
    tasks: await loadTaskSlice(root), // A33：第⑥视图承接对账（契约 §11）
  };
...
export async function checkReviewFresh(root, cliRoot, outRel = DEFAULT_OUTPUT) {
  ...
  const html = renderReview({
    projectName: path.basename(root), spec, contracts,
    apps: apps.apps.length > 0 ? apps : null,
    tasks: await loadTaskSlice(root), // A33：与 run 同口径，防新鲜度判定两张皮
  });
  const outPath = path.resolve(root, outRel);
...
/**
 * A33 任务切片（契约 §11）：只取承接对账所需字段，**刻意不含 status/retryCount/updatedAt**
 * （D-A33-01：运行态入渲染产物会让每次任务状态翻转都触发 A12 新鲜度机检「漂移」）。
 * docs/tasks/ 缺失或不可解析（TASK_FM/YAML_PARSE）→ null，第⑥视图如实标注承接对账缺席。
 */
async function loadTaskSlice(root) {
  if (!(await fileExists(path.join(root, 'docs', 'tasks')))) return null;
  let tasks;
  try {
  ...
  return tasks.map((t) => ({
    ...
    apis: Array.isArray(t.fm.apis) ? t.fm.apis : null,
  }));
}
