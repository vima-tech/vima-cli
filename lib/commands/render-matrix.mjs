 ...
 */
export function renderMatrix({ spec, contracts, tasks }) {
  // 接口 → 契约文件
  ...
  const out = [
    ...
    '> 口径：页面（需求） → 接口 → 契约 → 承接任务。V-COV-01 要求任何数据行都不得有空单元格。',
    '',
    '| 需求 | 接口 | 契约 | 任务 |',
    '|---|---|---|---|',
    ...rows.map((r) => `| ${r.join(' | ')} |`),
    '',
    `合计：${rows.length} 个页面 / ${rows.reduce((n, r) => n + Number.parseInt(r[1], 10), 0)} 条页面接口引用 / `
...
export async function run(argv, ctx) {
  ...
  const text = renderMatrix({ spec, contracts, tasks });
