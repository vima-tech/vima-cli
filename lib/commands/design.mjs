...
import { EXIT, VimaError, usageError, usageFromParseArgs, precondition } from '../util/errors.mjs';
...
async function cmdStatus(root, { check }, ctx) {
  ...
  if (check) {
    ...
    if (current !== text) {
      process.stderr.write(`vima design: DESIGN_INDEX_DRIFT: ${INDEX_REL} 与 spec/设计目录现状不一致（跑 vima design status 重生成）\n`);
...
export async function run(argv, ctx) {
  ...
  switch (sub) {
    ...
    default: throw new VimaError('UNKNOWN_SUB', `未知子命令 ${sub}`, { exitCode: EXIT.USAGE });
  }
}
