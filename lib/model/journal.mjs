...
export const V_INT_RULES = ['V-INT-01', 'V-INT-02', 'V-INT-03', 'V-INT-04', 'V-INT-05', 'V-INT-06'];
...
export async function collectReports(root) {
  ...
  const verification = {
    reports: 0, maxRound: 0, points: 0, failedPoints: 0, waived: 0, ngViolations: 0,
    // A42 D-A42-04：契约缺口与契约违规分开计。gaps **不计 failedPoints**
    // ——它记的是「规格本身有缺口，实现侧已做合规处置」，把它算成未过点位
    // 会让正确的工程处置变成永久 fail（page-68-fe 实证）。它的去处是收口清单。
    contractGaps: 0,
  };
  const shared = { changeRequests: 0 };
  // A46 D-A46-04：涌现决策计数（builder 报告 emergentDecisions 的合形条目）。
  // 只取计数不取明细——retro 是默认脱敏的消费方（决策文本可能含业务语义）。
  const decisions = { emergentA: 0, emergentB: 0 };
  let runtimeErrors = 0;
  ...
  for (const name of names) {
    ...
    if (name.endsWith('-verifier.json')) {
      ...
      const taskId = typeof data.taskId === 'string' && data.taskId !== ''
        ? data.taskId : name.replace(/-verifier\.json$/, '');
      for (const g of Array.isArray(data.contractGaps) ? data.contractGaps : []) {
        verification.contractGaps += 1;
        // 缺口条目允许是字符串或带 issue 字段的对象（角色模板两种写法都出现过）
        const label = typeof g === 'string' ? g
          : (g && typeof g === 'object' && typeof g.issue === 'string' ? g.issue : '');
        openPoints.push({ taskId, point: label, kind: 'contractGap' });
      }
      continue;
    }
    if (name.endsWith('-builder.json')) {
      ...
      // 容错读取（聚合器口径，同 contractGaps 的字符串/对象双形容忍）：
      // 只统计合形条目，非法条目静默跳过——形状纪律由 hook 守，这里是事后聚合。
      for (const d of Array.isArray(data?.emergentDecisions) ? data.emergentDecisions : []) {
        ...
        if (d.cls === 'A') decisions.emergentA += 1;
        else if (d.cls === 'B') decisions.emergentB += 1;
      }
    }
  }
