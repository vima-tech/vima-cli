...
function validateFrontmatter(fm, rel) {
  ...
  // 可选字段 app（A16）：任务归属端 id；多端必填/端册合法性/side 约束由 V-TASK-10 交叉校验
  if (fm.app !== undefined && (typeof fm.app !== 'string' || fm.app === '')) {
    throw taskError('app 必须是非空字符串（端册 app id，A16）', rel);
  }
  // 可选字段 conflictsWith（A8）：与这些任务共享代码路径，plan 保证不同批并行
  if (fm.conflictsWith !== undefined
    && (!Array.isArray(fm.conflictsWith) || fm.conflictsWith.some((d) => typeof d !== 'string'))) {
    throw taskError('conflictsWith 必须是任务 ID 字符串数组', rel);
  }
  // 可选字段 apis（A18）：本任务负责的契约接口子集；缺省 = 契约全集。
  // 归属闭环（⊆ 契约 / 不相交 / 全声明时并集=全集）由 V-TASK-12 交叉校验，此处只把结构关
  if (fm.apis !== undefined) {
    if (!Array.isArray(fm.apis) || fm.apis.length === 0
      || fm.apis.some((a) => typeof a !== 'string' || a.trim() === '')) {
      throw taskError('apis 必须是非空的接口字符串数组（形如 "GET /api/foods"，A18）', rel);
    }
  }
}
...
export async function loadTasks(root) {
  ...
  for (const name of names) {
    const task = await loadTaskFile(dir, name);
    const { fm } = task;
    const rel = task.file;
    if (byId.has(fm.taskId)) {
      throw taskError(`taskId "${fm.taskId}" 重复（首次声明于 ${byId.get(fm.taskId)}）`, rel);
    }
    byId.set(fm.taskId, rel);
    tasks.push(task);
  }
  return tasks;
}

/** 解析单个任务文件；严格与容错加载器共用，避免两套 frontmatter 口径漂移。 */
async function loadTaskFile(dir, name) {
  ...
  return { file: rel, path: abs, id: fm.taskId, fm, body };
}

/**
 * status 等只读观察口使用的容错加载：坏文件逐个隔离，其余任务仍返回。
 * validate/plan 等闸门继续使用上方严格 loadTasks，二者共享同一个单文件解析器。
 *
 * @returns {Promise<{tasks: Array<object>, issues: Array<{file:string,code:string,message:string}>}>}
 */
export async function loadTasksTolerant(root) {
  ...
  try {
    ...
    if (err.code === 'ENOENT') return { tasks: [], issues: [] };
    return {
      tasks: [],
      issues: [{ file: 'docs/tasks', code: err.code ?? 'TASK_READ', message: err.message }],
    };
  }
  names = names
    .filter((n) => n.endsWith('.md') && !n.startsWith('_') && n !== 'README.md')
  ...
  for (const name of names) {
    ...
    try {
      const task = await loadTaskFile(dir, name);
      if (byId.has(task.id)) {
        throw taskError(`taskId "${task.id}" 重复（首次声明于 ${byId.get(task.id)}）`, rel);
      }
      byId.set(task.id, rel);
      tasks.push(task);
    } catch (err) {
      issues.push({
        file: err.path ?? rel,
        code: err.code ?? 'TASK_READ',
