...
export const BUILTIN_KINDS = {
  'admin-web': {
    layoutVocab: ['toolbar', 'search', 'table', 'form', 'cards', 'tabs', 'pagination', 'steps', 'collapse', 'anchor'],
    regions: true,
    shell: 'desktop-admin',
    status: 'stable',
  },
...
function assertSafeRelativePath(value, label, { allowDot = false } = {}) {
  ...
  if (absolute || (!allowDot && value === '.') || parts.some((part) => part === '..' || part === '')) {
    throw new VimaError('APP_PATH', `${label} 含不安全路径 "${value}"（只允许项目根内的相对路径）`, {
...
function validateRoster(roster) {
  for (const app of roster.apps) {
    assertSafeRelativePath(app.dir, `端 ${app.id} dir`, { allowDot: true });
    assertSafeRelativePath(app.codeDir, `端 ${app.id} codeDir`);
    for (const dir of app.sharedDirs) assertSafeRelativePath(dir, `端 ${app.id} sharedDirs`);
  }
  if (roster.backend) {
    assertSafeRelativePath(roster.backend.dir, 'backend.dir');
    for (const dir of roster.backend.sharedDirs) assertSafeRelativePath(dir, 'backend.sharedDirs');
  }
  return roster;
}
...
export async function resolveApps(root, { cliRoot } = {}) {
  ...
  if (manifest && Array.isArray(manifest.apps)) {
    ...
    const backend =
      manifest.backend && typeof manifest.backend === 'object'
        ...
        : null;
    return validateRoster({ multi: apps.length > 1, apps, backend, kinds });
  }
    ...
    if (Array.isArray(template.apps) && template.apps.length > 0) {
      ...
      const backend =
        template.backend && typeof template.backend === 'object'
          ...
          : null;
      return validateRoster({ multi: false, apps: [app], backend, kinds });
    }
  ...
  return validateRoster({ multi: false, apps: [], backend: null, kinds });
}
