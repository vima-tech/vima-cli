...
export async function blockAdd(ctx, id) {
  ...
  // 拿错端当场说（R11 判据第三条）。**不拦**：端可以后补，装块与登记端谁先谁后是人的顺序，
  // 拦了就是替人决定顺序（C4 不阻塞）。判定复用 registry.sideMatchOf——判据只有一处。
  const kinds = [...new Set((updated.apps ?? []).map((a) => a?.kind).filter(Boolean))].sort();
  const side = registry.sideMatchOf(block.meta, kinds);
  if (side.sideMatch === 'mismatch') {
    notes.push(
      `这个块适用端是 ${side.sides.join(' / ')}，而本项目登记的端是 ${kinds.join(' / ')}`
        + '——现在没有任何端消费得了它。要么 vima app add 把那个端补上，要么这块装错了。'
        + '（不拦你：端可以后补。vima audit 会以 block-side-mismatch 一直盯着。）',
    );
  }
 ...
 */
/**
 * 实际并发（R5 判据前半句「实际并发数**可观测**」）。
 *
 * 并发 = 此刻持有**未过期**租约的执行者数。它只能从租约算，不能从 run 事件算：
 * 事件流回答「发生过什么」，此刻谁在干活是租约的职责。
 *
 * **边界必须随数字一起呈现，不能只写在文档里**：租约落在 `vima` 解析到的那个根，
 * 各 worktree 里的租约互相看不见、也就互不排除（ARCHITECTURE「已知未接线」第 4 条）。
 * 所以这个数字只在「所有执行者指向同一个根」时是真的——一个不说明适用条件的并发数，
 * 比没有更危险：它会被当成「并行已经做完了」。
 *
 * R5 判据后半句「与计划并发数可对账」在 v4 里**无对应物**（不是批次调度模型，
 * 没有「计划并发」这个对象），已改判，见 docs/requirements-baseline.md 的改判台账。
 */
async function concurrencyOf(root, now) {
  const { leases, corrupt } = await lease.list(root);
  const active = leases.filter((l) => lease.isActive(l, now));
  ...
  return {
    ...
    expired: leases.length - active.length,
    holders,
    corrupt,
    // 唯一真源就是这句话本身：呈现层原样转述，不各写各的
    scope: '只统计本项目根下的租约——各 worktree 的租约互不可见，'
      + '这个数字只在全部执行者指向同一个根时是真的',
  };
}

export async function status({ cwd = process.cwd(), env = process.env, now } = {}) {
  ...
  return {
    ...
    concurrency: await concurrencyOf(root, now ?? new Date()),
    stale,
    corrupt: p.corrupt,
  };
}
...
export async function audit(ctx) {
  ...
  if (registry) {
    ...
    for (const b of assets.blocks) {
      // 「拿错端的块是机检项」（R11 判据第三条）。判定在 registry.sideMatchOf，这里只格式化。
      // 与 block-broken 分开报：一个是「块坏了」，一个是「块好好的，但装错项目了」，
      // 处置完全不同（前者修资产仓，后者卸块或补端）。
      if (b.sideMatch === 'mismatch') {
        findings.push({
          kind: 'block-side-mismatch', severity: 'warn', subject: b.id,
          message: `块适用端 ${b.sides.join(' / ')}，而本项目登记的端是 ${assets.projectKinds.join(' / ')}`
            + '——没有任何端消费得了它，供料等于没装。vima block remove 卸掉，或 vima app add 把那个端补上',
          detail: { sides: b.sides, projectKinds: assets.projectKinds },
        });
      }
      if (b.ok) continue;
      findings.push({
    ...
    // 「没查」不是「查了没问题」——但也**不是每种没查都值得报**。
    // 项目还没登记端时全部块都判不了，那是新项目的正常状态（同 syncRules 的 'no-apps' 口径）：
    // 报出来就是一条每个新项目必亮、永远消不掉的告警，而那种告警会训练人忽略整张表。
    // 只报另一种：项目**已经**有端了，块却没声明 sides——那是资产仓可修的缺陷，
    // 且它让这条机检对该块整个失效。数据两种都在 summary.assets 里，不靠告警传递。
    const noSides = assets.projectKinds.length === 0
      ? []
      : assets.blocks.filter((b) => b.sideMatch === 'unchecked' && b.ok);
    if (noSides.length > 0) {
      findings.push({
        kind: 'closure-unchecked', severity: 'warn', subject: '(block-sides)',
        message: `${noSides.length} 个块没声明 sides，「有没有装错端」这一侧对它们没判：`
          + `${noSides.map((b) => b.id).join('、')}——去 block.json 补 sides，否则这条机检对它们恒不生效`,
        detail: { blocks: noSides.map((b) => b.id), projectKinds: assets.projectKinds },
      });
    }
