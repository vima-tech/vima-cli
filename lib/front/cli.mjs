...
function renderStatus(r) {
  ...
  // 实际并发（R5）。**边界与数字同屏**：一个不说明适用条件的并发数会被读成
  // 「并行已经做完了」，而跨 worktree 的租约互不可见。口径转述自 actions，不另写一份。
  const c = r.concurrency;
  if (c) {
    lines.push('', `  并发   ${c.active} 个执行者在干活${c.expired ? `（另有 ${c.expired} 份租约已过期）` : ''}`);
    for (const h of c.holders) {
      lines.push(`    ${h.claimId}  ← ${h.actor ?? '(未署名)'}${h.worktree ? ` @ ${h.worktree}` : ''}  到期 ${h.expiresAt ?? '?'}`);
    }
    lines.push(`    · ${c.scope}`);
    if (c.corrupt.length) lines.push(`    ! ${c.corrupt.length} 份租约读不出来（已跳过）：${c.corrupt.join('、')}`);
  }
  if (r.stale.length) {
