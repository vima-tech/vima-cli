...
- [ ] **V-TASK-07**（warn，A42 D-A42-01 改为按页聚合）：任务点覆盖度——**同一 `page` 的
  全部任务**的验收清单复选框数之**和**不得少于该页任务点数（交互 [items 带 action +
  rowActions] + 弹窗字段），少于则**按页报一条**（列出参与任务与各自条数）。
  按页聚合而非逐任务判：合并页（一页拆多个任务）是 vima 支持的形态，逐任务判会产生
  **永远消不掉的 warn**——实证 PAGE-03 五片 `24+22+28+32+27=133` 与页面点数完全一致、
  双向差集为空，却报 4 条。永远无法清除的告警会训练人忽略整张告警表（同 V-TASK-11 的 done 豁免口径）。
...
- [ ] **V-COV-01**（error）：文件存在，表格 ≥3 列，每行列数须与表头一致，任何数据行不得有空单元格或 `TODO`（缺口）。
  ...
  **逐表校验**（A44）：矩阵含两张表——「页面承接」与「业务规则承接」，各自独立比对列数。
- [ ] **V-COV-02**（warn）：末列（承接任务）为「—」开头的行逐条点名——该需求/规则**尚无任务负责**。
  恒 warn 不阻断（PLANNING 期尚未派任务是正常中间态），但不许静默留着：
  要么补任务，要么在 spec 第八章记一条「本期不承接」的决策。
  本条补的是 V-COV-01 的盲区：渲染器把「尚无任务承接」写成非空破折号串，
  「不得有空单元格」对真缺口恒不命中。
...
- [ ] **V-CODE-03**（error，A42 D-A42-02）：错误码对账——带 `@vima` 标注的后端文件里
  `BusinessException(ErrorCode.X)` 的数字码必须 ∈ 该任务 `contract` 指向契约的 `errors[].code` 全集。
  **只做「实现抛了、契约没声明」一个方向**：反向（契约声明了、实现不抛）无法零假阳性
  ——错误码常在 Service 深层抛、`@vima` 标注只到文件级、且骨架的统一异常处理器会集中转
  40001/403，反向判定会让模板自身满屏报错。归属取契约 module 全集而非任务 `apis` 子集
  （Service 常被同模块兄弟子任务共用）。找不到 `ErrorCode` 枚举时整条跳过。
- [ ] **V-CODE-04**（error / warn，A42 D-A42-02）：权限点三面对账——spec
  `vima:menus[].features[].perms` 声明的每个权限点，必须在**后端**有 `@perm.has('X')`
  （缺 = **error**，越权风险），且在该菜单归属端的**前端**有 `v-auth="X"`
  （缺 = **warn**，入口不设防、点下去才吃 403）。某一面没有任何带标注的业务代码时该面不判
  （规格期/该端未开工不误报）；未声明 `perms` 的项目完全不触发。**不含菜单种子**
  ——种子是项目自建产物，vima 无法通用定位，加第四面是镀金。
- [ ] **V-SRC-02**（error，A42 D-A42-02）：编号引用校验——带 `@vima` 标注的业务代码
  （含 `data-page`/`data-block` 属性与注释）里出现的 `PAGE-\d+` / `MODAL-\d+` /
  `RULE-\d+` / `NG-\d+` 必须存在于 spec 的结构化数据块。存在性只取结构化块不看正文，
  故 spec 里声明为「空号/保留不复用」的编号天然算不存在。
  **优先级不低于前两条**：A22 字段级机检、A34 保真验收、converge 点位对账全都以
  `data-page` 为锚——锚点指向不存在的页面时，这些机制不是报错，而是**静默地什么都不检**。
  它同时拦住 builder 编造 NG/RULE 编号给自己发豁免（实证：`NG-16` 被用作
  「微信登录留桩」的依据，而 spec 止于 NG-15）。
...
- [ ] **V-INT-06**（warn，A42 D-A42-03）：**被读的必须有人写**——契约声明的响应字段，
  其同名 JPA `@Entity` 属性若在全仓无任何写入点（Lombok setter 调用 / 构造器 / `@Builder` 链 /
  `@PrePersist` 裸赋值 / JPQL `update`）⇒ 该字段恒空。整张表没有生产者时按实体报一条，
  否则逐属性一条。**行动项是「指派一个任务负责写入，或回契约删除这些响应字段」。**
  立项理由：任务拆分以**端点**为单位，而数据的生产者—消费者关系**跨端点**
  ——「谁往这张表写」不对应任何端点，于是不在任何任务的 `apis` 责任田里，
  V-INT-01「接口零实现」也查不到。实证（sustain-v4）：`mp_message` 表全仓无写入方
  且无任务被指派为生产者 ⇒ 消息中心恒空；`prescription.his_visit_id` 无写入点 ⇒
  按它 join 的就诊记录里处方**在任何数据下都是空数组**。
  四道收窄闸门（实测假阳性源）：接收者类型解析不出的链式 setter 按同名属性全局压制；
  `@RequestBody`/Jackson 绑定的类型整类跳过（反射写入，源码里没有写入点却完全正常）；
  框架托管列（`@Id`/`@GeneratedValue`/`@Version`/审计注解/`insertable=false`/带初始化值）不判；
  契约响应里没有同名字段的属性不判（没有「有人读」的证据，不算接缝失效）。
