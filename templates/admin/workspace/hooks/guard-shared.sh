#!/usr/bin/env bash
# guard-shared.sh —— 共享层写保护（PreToolUse hook，matcher: Write|Edit）
#
# 机制（设计文档 §10.7 策略二 / §16.3）：
#   - 从 stdin 读取 hook JSON，取 tool_input.file_path 判断写入目标
#   - 命中共享目录（src/components/ src/utils/ vendor/、
#     backend 路径含 /config/ 或 /security/，与 template.json sharedDirs 同步）
#     且 .vima/shared-write-token 缺失或已过期 → exit 2 阻断
...
process.stdin.on("end", () => {
  ...
  // 共享目录判定（与 template.json sharedDirs 同步）
  const frontendShared = ["src/components/", "src/utils/", "vendor/"];
  const hitFrontend = frontendShared.some((d) => rel.startsWith(d));
  const backendShared = ["/config/", "/security/"];
  const hitBackend = rel.startsWith("backend/") && backendShared.some((d) => rel.includes(d));

  // 契约保护（§9.5 契约纪律 4）：仅 DEVELOPING 阶段拦截 docs/contracts/** 写入
  let hitContracts = false;
  if (rel.startsWith("docs/contracts/")) {
    try {
      const lc = JSON.parse(fs.readFileSync(path.join(root, "docs", "lifecycle.json"), "utf8"));
      hitContracts = lc && lc.currentPhase === "DEVELOPING";
    } catch { /* lifecycle 缺失/损坏不误伤，放行 */ }
  }
  ...
  let reason;
  try {
    const expiry = fs.readFileSync(tokenPath, "utf8").trim();
