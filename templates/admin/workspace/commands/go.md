...
- **合法停点白名单（A17 反停顿纪律，A18 增加机读落盘）**：批次之间不得结束回合
  ...
  **依赖未满足且当前无其他可派批次**——如 pipeline 任务依赖尚未收敛的批次，跨 layer 屏障
  ...
  ④ 用户主动中断；⑤ **DESIGNING 期等待用户裁定设计方向**（A45 D-A45-02，
  写 `stopReason=design`）——把三个方向摆给用户后**必须结束回合等人**，
  这是白名单里唯一一条「产出没问题、就是要等人看一眼」的停点。
  > 为什么单列这一条：A0 三方向裁定按 A6 阶梯登记为〔L5·人审〕（C-A34-03），
  > 而本白名单此前只有「受阻才停」三类，没有「等裁定」这一类。sustain-v4 实测的结果是
  > Agent 自行选定胜者并把理由写成「以免设计闸门阻塞后续实现」——**它是在遵守反停顿纪律**。
  > 停下来等人在这里不是违纪，是本阶段的正确行为；`vima design approve direction`
  > 在非交互环境会直接抛 `DIRECTION_SELECTOR`，硬绕只能走 `--agent-selected --reason`，
  > 那会在此后每道闸门被念出来。
