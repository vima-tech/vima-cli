...
// guard-shared.sh 的保护面与 template.json sharedDirs 曾三处口径不一。
...
test('guard-shared.sh 保护面与 template.json sharedDirs 逐项同步（契约 §6.3 单一真源）', async () => {
  const guard = await readFile(path.join(ADMIN, 'workspace/hooks/guard-shared.sh'), 'utf8');
...
test('红线文案与 sharedDirs 同步：CLAUDE.project.md 与 vima-builder.md 覆盖全部前端共享目录', async () => {
  ...
  for (const rel of ['workspace/CLAUDE.project.md', 'workspace/agents/vima-builder.md']) {
...
// ── post-write.sh 区块标记机械对账（§13.3 hook 半，契约 §14）──
import { mkdtemp, mkdir, writeFile, rm } from 'node:fs/promises';
import { tmpdir } from 'node:os';
import { spawnSync } from 'node:child_process';

const HOOK = path.join(ADMIN, 'workspace/hooks/post-write.sh');
...
test('区块标记对账：标记齐全 → 放行；缺区块/缺弹窗 → exit 2 且指名', async (t) => {
  ...
  await writeFile(path.join(root, 'src/views/miss.vue'),
    '<template><div data-page="PAGE-01"><div data-block="search"/></div></template>');
...
test('区块标记对账：设计外区块 / 未知页面 → exit 2；无 data-page 或无 manifest → 跳过', async (t) => {
  ...
  await writeFile(path.join(root, 'src/views/extra.vue'),
    '<template><div data-page="PAGE-01"><div data-block="search"/><div data-block="table"/><div data-block="cards"/><M data-modal="MODAL-01"/></div></template>');
  const r1 = runHook(root, 'src/views/extra.vue');
  assert.equal(r1.status, 2);
  assert.match(r1.stderr, /设计外区块.*cards/);

  await writeFile(path.join(root, 'src/views/ghost.vue'), '<template><div data-page="PAGE-99"/></template>');
