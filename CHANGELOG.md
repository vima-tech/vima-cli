...
用需求基线冷读比对实现时发现的一处**上游缺陷**：`ARCHITECTURE.md` 的基线表是人从
基线原文抄过来的摘要，抄的时候**「判据」整列被丢掉了**——表里只剩「陈述 + 要点」。

后果不是文档不好看。基线原文的立身之本就是那句「每条附**判据**——怎么算满足，
而不只是想要什么」；判据一丢，「这条做到没有」就退回成读着办。逐条比对查实：
**丢掉判据的三条，实现也一并没人做**，而没有任何人发现——

| 丢掉的判据 | 当时状态 |
|---|---|
| R5「实际并发数**可观测**，且与计划并发数可对账」 | 零呈现（被摘要改写成「靠隔离」，那是解决方案不是需求） |
| R7「四层各有封闭词表 · **骨架自身也满足，不得豁免自己**」 | 工具自己不消费自己的词表 |
| R11「加块不改内核 · 不含 L4 也能消费 · **拿错端的块是机检项**」 | 前两条已满足，第三条零实现 |

`docs/v4-optimization-recommendations.md` 的 P0-6 其实已经要求过「每条 R/C 包含：
陈述、动机、**验收结果**、明确不做」——基线表补上了，验收结果那一列没补。本轮补完。

### 新增

- **`docs/requirements-baseline.md`** —— 需求基线**正本**进仓库。此前它只以摘要形式
  存在于 `ARCHITECTURE.md`，最上游那份依据反而在仓库外（一个链接），谁也 review
  不了它改没改（C3 要求真源可读可 diff 可 review）。正本含每条的判据、裁定、边界、
  实证，以及**落地状态三档**（✅ 已兑现 / ⚠️ 判据成立但欠着 / 📝 已改判 + 理由）。
- **`registry.sideMatchOf(meta, projectKinds)`** —— R11 判据第三条「拿错端的块是机检项」
  的唯一判据处。块在 `block.json` 声明 `sides`，项目在 `config.apps[].kind` 登记端，
  两边完全无交集 = 装了个本项目没有任何端消费得了的块。
  **三态不是布尔**：`match` / `mismatch` / `unchecked`——「没查」必须和「查了没问题」
  分得开。零假阳性靠第三态：任何一边说不出话一律不判。
- **`audit` 新发现 `block-side-mismatch`（warn）** —— 与 `block-broken` 分开报：
  一个是「块坏了」，一个是「块好好的，但装错项目了」，处置不同（修资产仓 vs 卸块/补端）。
  `block add` 当场也给一条提示，但**不拦**——端可以后补，拦了就是替人决定顺序（C4）。
  **项目还没登记端时刻意不报任何东西**：那是新项目的正常状态，报出来就是一条
  每个新项目必亮、永远消不掉的告警，而那种告警会训练人忽略整张表。
- **`vima status` 报实际并发** —— R5 判据前半句。`concurrency` =
  `{ active, expired, holders, corrupt, scope }`，只能从**租约**算（事件流回答
  「发生过什么」，此刻谁在干活是租约的职责）。**边界与数字同屏**：`scope` 说明
  跨 worktree 的租约互不可见，这个数字只在全部执行者指向同一个根时是真的——
  一个不说明适用条件的并发数会被读成「并行已经做完了」。
- **`z.seams` 新增一条：基线表每条 R#/C# 的判据列必须非空，且正本必须在仓库里。**
  判据被抄丢过一次，机检是唯一能防第二次的东西。已做变异验证：挖空任一条判据、
  或删掉指向正本的引用，都当场红。

### 变更

- **R5 判据后半句「与计划并发数可对账」改判为不适用**（记进正本的改判台账）。
  v4 不是批次调度模型，没有「计划并发」这个对象；计划侧留待真出现调度器时再立。
  改判**写下来**而不是静默丢掉——静默丢掉正是这次要治的病。
- `ARCHITECTURE.md` 基线表补「判据」与「落地」两列，并指向正本；
  `CLAUDE.md` 的设计依据指向正本。
- `ARCHITECTURE.md`「已知未接线」新增一条：**供料的「料」几乎是空的**。机制侧齐了
  （注册表 / lock / `block add` / `next` 按层下发），但资产仓只有 1 个块，
  `blocks/common/` 与 `blocks/wechat/` 是空目录。机制齐了最容易被读成「这条做完了」，
  而实际拿它起项目时 `vima block list` 只有一个选项。

### 已知欠账（如实记，不假装满足）

- R7「骨架自身也满足，不得豁免自己」：vima 自己的 Web UI 不消费那三份词表。
- R7 的 Claude Design 两档设计稿工序未接线；词表初值是人工拟定，
  不是判据要求的「由既有项目实际写法归一化统计得出」。
- R1「冷读作度量」没有采集口。
- R5 的并发数暂不进 Web 观测面：判活租约要读系统时钟，与 `front/web.mjs`
  「不读系统时钟」的渲染纪律冲突，两者未裁，本轮不硬塞。
